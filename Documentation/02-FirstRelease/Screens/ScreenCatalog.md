...
1400×800 px — стандартный размер панели для окон игровых механик: `Station`, `Trade`, `Hire`, `Contracts`, `Cargo`, `Finance`, `Loot`, `Ship`, `Character Communication`, `Dialog`. На этот размер переведены `Station`, `Trade`, `Hire`, `Contracts`, `Finance` (`FinanceLayout.PanelWidth/PanelHeight`) и `Ship` (`ShipLayout.PanelWidth/PanelHeight`).
...
| `Trade` | Реализован MVP покупки, продажи и заправки; требует переименования в `ScreenTrade` | `src/DeepSpaceSaga.Client/UI/Screens/Trade/TradeScreen.cs` | Локальный рынок станции: покупка, продажа и заправка `Fuel`; динамическая экономика и профили станций требуют реализации. | Из `Station` (кнопка `TRADE`, вложенный modal); закрытие возвращает в `Station`. |
...
4. `Station` открывается автоматически после успешного `navigation.dock` либо повторным кликом по станции/кораблю игрока и является hub-экраном для `Trade`, `Hire`, `Contracts` и `Finance` — все четыре кнопки (`TRADE`/`HIRE`/`CONTRACTS`/`FINANCE`) реализованы; `Trade` открывает реализованный MVP торгового экрана, `Hire`/`Contracts` остаются заглушками, `Finance` открывает уже существующий `FinanceScreen` (тот же, что открывается с панели механик сессии). `Contracts` выделен из исходного `Hire` (наём экипажа vs. пассажирские контракты).
