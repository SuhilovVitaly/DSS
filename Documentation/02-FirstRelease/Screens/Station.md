...
- В целевой версии получает из authoritative snapshot роль станции, краткий статус рынка и доступные станционные сервисы.
...
`Trade` остаётся единственным экраном операций рынка. Станция не открывает отдельный экран маршрутов или глобальной торговли.
...
- Повторное открытие левым кликом по карте — по станции, к которой корабль пристыкован, либо по самому кораблю игрока, пока он пристыкован (маркеры физически почти совпадают после дока, см. `Documentation/02-FirstRelease/Mechanics/Docking.md` и приоритет выбора объекта в `ScreenCatalog.md`/§54.9 основного requirements-документа).
