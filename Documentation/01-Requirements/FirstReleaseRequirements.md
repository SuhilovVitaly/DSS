...
Основной источник истины по движку: `Documentation/01-Requirements/EngineRequirements.md`.

Назначение документа: собрать границы первого релиза DSS как минимального игрового скелета. Этот файл описывает релиз в целом; детализация экранов и механик хранится в `Documentation/02-FirstRelease/`.
...
- `Documentation/02-FirstRelease/Screens/MainMenu.md` - главное меню.
- `Documentation/02-FirstRelease/Screens/ScenarioSelect.md` - экран выбора сценария новой игры.
- `Documentation/02-FirstRelease/Screens/ScreenCatalog.md` - минимальный набор окон первого релиза и связи между ними.
- `Documentation/02-FirstRelease/Screens/GameSession.md` - экран игровой сессии и тактическая карта.
- `Documentation/02-FirstRelease/Screens/Station.md` - экран пристыкованной станции.
- `Documentation/02-FirstRelease/Screens/Trade.md` - экран торговли.
- `Documentation/02-FirstRelease/Screens/GameMenu.md` - игровое меню поверх сессии.
- `Documentation/02-FirstRelease/Screens/Settings.md` - настройки.
- `Documentation/02-FirstRelease/Screens/Save.md` - окно сохранения.
- `Documentation/02-FirstRelease/Screens/Load.md` - окно загрузки.
- `Documentation/02-FirstRelease/Screens/Dialog.md` - универсальное окно диалога.
- `Documentation/02-FirstRelease/Screens/Ship.md` - окно корабля.
- `Documentation/02-FirstRelease/Screens/Cargo.md` - окно грузового отсека.
- `Documentation/02-FirstRelease/Screens/Loot.md` - окно подбора/добычи предметов.
- `Documentation/02-FirstRelease/Screens/CharacterCommunication.md` - окно общения с персонажами.
- `Documentation/02-FirstRelease/Screens/Finance.md` - окно финансов.
- `Documentation/02-FirstRelease/Screens/Hire.md` - окно найма экипажа.
- `Documentation/02-FirstRelease/Screens/Contracts.md` - окно пассажирских контрактов (выделено из `Hire`).
- `Documentation/02-FirstRelease/TechnicalTasks/ScreenNamingConventionRefactor.md` - техническое задание на приведение существующих экранов к naming convention.
- `Documentation/02-FirstRelease/TechnicalTasks/StationEconomyProductionAndSizing.md` - техническое задание на размер станции, производящие модули, ценовые коэффициенты и упаковки продажи.
- `Documentation/02-FirstRelease/Mechanics/TetrarchClass.md` - стартовый корабль игрока Tetrarch Class.
- `Documentation/02-FirstRelease/Mechanics/CommandPanels.md` - смысловые командные панели корабля.
- `Documentation/02-FirstRelease/Mechanics/TacticalMapAndManeuvering.md` - тактическая карта и маневрирование.
- `Documentation/02-FirstRelease/Mechanics/Docking.md` - стыковка.
- `Documentation/02-FirstRelease/Mechanics/Trading.md` - торговля на станциях.
- `Documentation/02-FirstRelease/Mechanics/StationInventory.md` - конечный склад станции.
- `Documentation/02-FirstRelease/Mechanics/Money.md` - `Credits` и стартовые балансы.
- `Documentation/02-FirstRelease/Mechanics/CargoHold.md` - грузовой отсек.
- `Documentation/02-FirstRelease/Mechanics/CrewAndHabitation.md` - экипаж, пассажиры и жилой модуль.
- `Documentation/02-FirstRelease/Mechanics/CrewDialogues.md` - экипаж и диалоги.
- `Documentation/02-FirstRelease/Mechanics/PassengerContracts.md` - пассажирские контракты на станциях.
- `Documentation/02-FirstRelease/Mechanics/StationDialogues.md` - диалоги с представителями станций.
- `Documentation/02-FirstRelease/Mechanics/IceMining.md` - добыча льда.
- `Documentation/02-FirstRelease/Mechanics/Fuel.md` - топливо двигателя.
- `Documentation/02-FirstRelease/Mechanics/ElectricityAndEnergyCells.md` - реактор, электричество и `Energy Cells`.
...
- `navigation.dock` теперь настоящая authoritative команда (`SimulationEngine.TryStartNavigationCommand`), а не catalog-only заглушка: проверяет target/дистанцию (`rangeKm` из command definition)/синхронизацию, при успехе синхронизирует корабль со станцией и выставляет `IsDocked`/`DockedStationObjectId` (персистентны через save/load). `StationScreen` (заглушка, по паттерну `Finance`/`Ship`) открывается автоматически после успешного дока или повторным кликом по станции/кораблю игрока. Отстыковка не реализована. Детали: `Documentation/02-FirstRelease/Mechanics/Docking.md`, `Documentation/02-FirstRelease/Screens/Station.md`.
...
- Кнопка `TRADE` на `StationScreen` реальная (не placeholder-строка): открывает реализованный `TradeScreen` вложенным modal поверх `Station`.
...
- Текущие частично/полностью реализованные экранные классы пока не соответствуют новой конвенции, потому что используют суффикс `Screen`; их переименование описано в `Documentation/02-FirstRelease/TechnicalTasks/ScreenNamingConventionRefactor.md`.
...
|---|---|---|---|
| `Main Menu` | Частично реализовано: `MainMenuScreen`; `Load` нарисован, но отключен. | `Documentation/02-FirstRelease/Screens/MainMenu.md` | `Load`, `Settings`, `Scenario Select`. |
| `Scenario Select` | Реализовано: `ScenarioSelectScreen`. | `Documentation/02-FirstRelease/Screens/ScenarioSelect.md` | Из `Main Menu` по `New Game`; выбор сценария открывает `Session`; `BACK`/`Escape` возвращают в `Main Menu`. |
| `Load` | Не реализовано отдельным экраном; есть disabled-кнопка `LOAD` в `MainMenu` и `GameMenu`. | `Documentation/02-FirstRelease/Screens/Load.md` | Из `Main Menu` и `Game Menu`; после выбора сохранения открывает `Session`. |
| `Settings` | Частично реализовано: `SettingsScreen`. | `Documentation/02-FirstRelease/Screens/Settings.md` | Из `Main Menu` и `Game Menu`; закрытие возвращает на предыдущий экран. |
| `Session` | Частично реализовано как `GameSessionScreen`. | `Documentation/02-FirstRelease/Screens/GameSession.md` | Открывает `Game Menu`, `Station`, `Ship`, `Loot`, `Character Communication`, `Cargo`, `Dialog`. |
| `Game Menu` | Частично реализовано: `GameMenuScreen`; `Load` и `Settings` нарисованы, но пока отключены. | `Documentation/02-FirstRelease/Screens/GameMenu.md` | `Save`, `Load`, `Settings`, возврат в `Session`, выход в `Main Menu`. |
| `Save` | Реализовано как `SaveScreen`. | `Documentation/02-FirstRelease/Screens/Save.md` | Из `Game Menu`; закрытие возвращает в `Game Menu`. |
| `Dialog` | Не реализовано. | `Documentation/02-FirstRelease/Screens/Dialog.md` | Из `Session` для простых линейных диалогов. |
| `Station` | Реализована заглушка: `StationScreen`. | `Documentation/02-FirstRelease/Screens/Station.md` | Из `Session` автоматически после успешного `navigation.dock`, либо повторным кликом по станции/кораблю игрока; закрытие возвращает в `Session`. Планируется открывать `Trade`, `Hire`, `Finance`. |
| `Ship` | Не реализовано. | `Documentation/02-FirstRelease/Screens/Ship.md` | Из `Session`; открывает `Character Communication`. |
| `Loot` | Не реализовано. | `Documentation/02-FirstRelease/Screens/Loot.md` | Из `Session`; используется для результатов добычи/подбора. |
| `Character Communication` | Не реализовано. | `Documentation/02-FirstRelease/Screens/CharacterCommunication.md` | Из `Session` и `Ship`; показывает общение с членами экипажа и персонажами. |
| `Cargo` | Не реализовано. | `Documentation/02-FirstRelease/Screens/Cargo.md` | Из `Session`; показывает грузовой отсек корабля. |
| `Finance` | Реализована заглушка: `FinanceScreen`. | `Documentation/02-FirstRelease/Screens/Finance.md` | Из `Session` (кнопка `F`/`Ctrl+F`) или из `Station` (кнопка `FINANCE`, вложенный modal); закрытие возвращает туда, откуда открыли. Финансовые станционные операции/сводка не реализованы. |
| `Hire` | Реализована заглушка: `HireScreen`. | `Documentation/02-FirstRelease/Screens/Hire.md` | Из `Station` (кнопка `HIRE`, вложенный modal); закрытие возвращает в `Station`. Наём экипажа не реализован (полностью за рамками первого релиза). |
| `Contracts` | Реализована заглушка: `ContractsScreen`. | `Documentation/02-FirstRelease/Screens/Contracts.md` | Из `Station` (кнопка `CONTRACTS`, вложенный modal); закрытие возвращает в `Station`. Выделено из `Hire`; пассажирские контракты не реализованы. |
| `Trade` | Реализован MVP: `TradeScreen` + authoritative операции `trade.buy`/`trade.sell`/`trade.refuel`; требует доработки под новую модель размера станции и упаковок продажи. | `Documentation/02-FirstRelease/Screens/Trade.md` | Из `Station` (кнопка `TRADE`, вложенный modal); закрытие возвращает в `Station`. |
...
- Стартовая станция имеет размер `Large` / `Большой`.
...
- Fallback-генерация товаров станции `20..500` применяется только к станциям без явного склада.
- Номенклатура торговли делится на `Resource`, `Good` и `Module`.
- Продажа `Resource` выполняется упаковками по `100` kg, продажа `Good` - упаковками по `10` kg, продажа `Module` - поштучно.
...
- Цены товаров считаются от базовой цены и всех применимых факторов станции: размер станции, события/бафы/дебафы, производственная потребность станции.
...
- Базовые цены для `Food Rations` и продаваемых модулей требуют балансного решения.
...
Детали: `Documentation/02-FirstRelease/Mechanics/Docking.md`.
...
Детали: `Documentation/02-FirstRelease/Screens/Station.md`.

### Торговая система

В пристыкованном состоянии на станциях игрок может покупать и продавать ресурсы/товары, включая `Energy Cells` и лед, а также пополнять `Fuel` в баках двигателя. Торговля учитывает cargo capacity, конечный склад станции, `Credits` игрока, скрытый баланс `Credits` станции и итоговые цены `Round(basePrice * allApplicableStationFactors)`.

MVP торговли уже реализован как authoritative Engine + Contracts + Client. Новая модель размеров станции, упаковок продажи и производящих модулей описана как следующая доработка.

Детали: `Documentation/02-FirstRelease/Mechanics/Trading.md` и `Documentation/02-FirstRelease/Screens/Trade.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/Money.md`.

### Склад станции

Склад станции конечный. Стартовая станция имеет размер `Large` / `Большой` и explicit inventory: `Food Rations = 500`, `Energy Cells = 350`, `Fuel = 700`, `Ice = 320`, `Iron Ore = 410`, `Silicon = 70`, `Magnesium Ore = 120`. Fallback-генерация `20..500` применяется только к станциям без явного склада. Цены используют базовую цену и все применимые факторы станции: размер, события/бафы/дебафы и производственную потребность. Базовые цены зафиксированы для ресурсов `RES-2001..RES-2004` и товаров `ITM-3001..ITM-3005`.

Детали: `Documentation/02-FirstRelease/Mechanics/StationInventory.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/CargoHold.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/ElectricityAndEnergyCells.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/CrewAndHabitation.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/CrewDialogues.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/StationDialogues.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/IceMining.md`.
...
Детали: `Documentation/02-FirstRelease/Mechanics/Fuel.md`.

## Открытые вопросы

На текущем уровне требований первого релиза открыты балансные вопросы по расширенной станционной экономике:

- базовые цены для `Food Rations` и продаваемых модулей;
- точная формула влияния коэффициента потребности производящего модуля на расход сырья и/или дополнительную цену;
- окончательное правило продажи `Fuel` игроком: как обычный `Good` упаковками по `10` kg или как отдельная fuel-операция.

Остальные балансировочные детали, явно вынесенные за рамки первого релиза, перечислены ниже.
