...
- `Docs/FirstRelease/TechnicalTasks/StationEconomyProductionAndSizing.md` - техническое задание на размер станции, производящие модули, ценовые коэффициенты и упаковки продажи.
...
| `Trade` | Реализован MVP: `TradeScreen` + authoritative операции `trade.buy`/`trade.sell`/`trade.refuel`; требует доработки под новую модель размера станции и упаковок продажи. | `Docs/FirstRelease/Screens/Trade.md` | Из `Station` (кнопка `TRADE`, вложенный modal); закрытие возвращает в `Station`. |
