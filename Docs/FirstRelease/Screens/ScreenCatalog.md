...
4. `Station` открывается автоматически после успешного `navigation.dock` либо повторным кликом по станции/кораблю игрока и является hub-экраном для `Trade` и `Hire` (кнопки `TRADE`/`HIRE` реализованы и открывают заглушки `TradeScreen`/`HireScreen`) и `Finance` (пока не реализован).
