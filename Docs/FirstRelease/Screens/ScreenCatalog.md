...
4. `Station` открывается автоматически после успешного `navigation.dock` либо повторным кликом по станции/кораблю игрока и является hub-экраном для `Trade` (кнопка `TRADE` реализована и открывает заглушку `TradeScreen`), `Hire` и `Finance` (последние два пока не реализованы).
