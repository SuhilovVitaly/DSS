...
1400×900 px — стандартный размер панели для окон игровых механик: `Station`, `Trade`, `Hire`, `Cargo`, `Finance`, `Loot`, `Ship`, `Character Communication`, `Dialog`. Первым на этот размер переведён `Finance` (`FinanceLayout.PanelWidth/PanelHeight`).
