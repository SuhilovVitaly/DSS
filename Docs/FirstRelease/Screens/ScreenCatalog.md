...
| `Hire` | Реализована заглушка (открытие/закрытие/пауза, без данных механик); требует переименования в `ScreenHire` | `src/DeepSpaceSaga.Client/UI/Screens/Hire/HireScreen.cs` | Найм/пассажирские контракты на станции. | Из `Station` (кнопка `HIRE`, вложенный modal); закрытие возвращает в `Station`. |
| `Trade` | Реализована заглушка (открытие/закрытие/пауза, без данных механик); требует переименования в `ScreenTrade` | `src/DeepSpaceSaga.Client/UI/Screens/Trade/TradeScreen.cs` | Покупка, продажа и заправка `Fuel`. | Из `Station` (кнопка `TRADE`, вложенный modal); закрытие возвращает в `Station`. |
...
4. `Station` открывается автоматически после успешного `navigation.dock` либо повторным кликом по станции/кораблю игрока и является hub-экраном для `Trade`, `Hire` и `Finance` — все три кнопки (`TRADE`/`HIRE`/`FINANCE`) реализованы; `Trade`/`Hire` открывают свои заглушки, `Finance` открывает уже существующий `FinanceScreen` (тот же, что открывается с панели механик сессии).
