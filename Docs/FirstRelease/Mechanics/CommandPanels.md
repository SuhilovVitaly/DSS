...
| `navigation.approach` | Engine | Выбран celestial object; сначала выводит корабль в точку позади цели по её текущему направлению, затем продолжает сближение до текущей точки цели; будущее линейное движение цели не экстраполируется, фактическое состояние перечитывается каждый цикл; команда выравнивает направление, но не меняет скорость — торможение остаётся игроку |
