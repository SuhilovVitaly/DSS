...
| `navigation.approach` | Engine | Выбран celestial object; сначала выводит корабль в точку позади цели по ходу её движения, затем продолжает сближение до самой цели; расстояние до промежуточной точки ограничивается текущей дистанцией и радиусом разворота корабля (`trailDistanceKm` задаёт максимум), точка пересчитывается каждый цикл, команда завершается у цели точным совпадением скорости и направления |
