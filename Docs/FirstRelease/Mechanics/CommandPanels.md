...
| `navigation.approach` | Engine | Выбран celestial object; в момент команды фиксируется текущая позиция и направление цели, затем строится непрерывная траектория с учётом радиуса разворота: корабль обходит цель, заходит ей в хвост и проходит зафиксированную точку тем же направлением; будущее движение цели не экстраполируется; скорость корабля команда не меняет — торможение остаётся игроку |
