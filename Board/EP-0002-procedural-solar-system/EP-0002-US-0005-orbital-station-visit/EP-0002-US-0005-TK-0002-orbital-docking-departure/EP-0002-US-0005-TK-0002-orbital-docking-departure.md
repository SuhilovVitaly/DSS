...
ticket: EP-0002-US-0005-TK-0002-orbital-docking-departure
title: Стыковка через диалог и непрерывный уход
stage: approved
layer: engine
depends_on: [EP-0002-US-0004-TK-0001-scenario-group-translation, EP-0002-US-0004-TK-0002-all-scenario-system-content, EP-0002-US-0005-TK-0001-orbital-synchronization]
files_touched: 4
...
# Стыковка через диалог и непрерывный уход

## Why

Посещение движущейся станции: стыковка через диалог и непрерывный уход даёт проверяемый шаг к результату истории. End state: полный путь и координаты до/после, offset соответствует контракту.; wrap угла, цель изменилась до effect, отказ атомарен.; орбита/портовые сборы используют свои временные базы.
...
| src/DeepSpaceSaga.Engine/Dialogue/DialogueEffectTransaction.cs | src/DeepSpaceSaga.Engine/Dialogue/DialogueEffectTransaction.cs:14 — public static string? Prepare(GameDataRegistry registry, ImmutableArray<SpaceObjectRuntime> objects, | Только актуальная docking binding/повторный owner gate; сохранить transactional rollback. |
...
| tests/DeepSpaceSaga.Engine.Tests/OrbitalDockingDepartureTests.cs | Новый файл; владелец EP-0002-US-0005-TK-0002-orbital-docking-departure. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change; navigation.dock/navigation.undock, DialogueEffectTransaction.Prepare и validateDock callback сохраняются.

## Implementation steps

1. Dock validation сравнивает heading по кратчайшей угловой разнице (wrap 359.999/0.001), speed epsilon сохраняется. Проверить актуальные позы одной эпохи и повторную validateDock внутри effect transaction.
2. DockPlayerToStation устанавливает orbital parent/offset=(1,1) вместе с IsDocked; Undock снимает binding и оставляет точную текущую позу, касательную и скорость в новый linear baseline. Очистить active cycles как существующий контракт; не вводить docking snapping сверх существующего offset.
3. Пройти настоящий docking dialogue (не устанавливать IsDocked тестовым читом), pause/resume, docked station travel и undock; проверить повтор команд и отмену диалога без списания сверх существующих правил.
...
Test class: `OrbitalDockingDepartureTests`. Тестовые сценарии и ожидаемые результаты:

- `OrbitalDockingDepartureTests.DialogueDockAndUndockOrbitContinuity` — полный путь и координаты до/после, offset соответствует контракту.
- `OrbitalDockingDepartureTests.DockHeadingWrapAndValidationRace` — wrap угла, цель изменилась до effect, отказ атомарен.
- `OrbitalDockingDepartureTests.DockedPauseTravelResume` — орбита/портовые сборы используют свои временные базы.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `OrbitalDockingDepartureTests.DialogueDockAndUndockOrbitContinuity`, `OrbitalDockingDepartureTests.DockHeadingWrapAndValidationRace`, `OrbitalDockingDepartureTests.DockedPauseTravelResume` |
| AC-0002 | `OrbitalDockingDepartureTests.DialogueDockAndUndockOrbitContinuity`, `OrbitalDockingDepartureTests.DockHeadingWrapAndValidationRace`, `OrbitalDockingDepartureTests.DockedPauseTravelResume` |
| AC-0003 | `OrbitalDockingDepartureTests.DialogueDockAndUndockOrbitContinuity`, `OrbitalDockingDepartureTests.DockHeadingWrapAndValidationRace`, `OrbitalDockingDepartureTests.DockedPauseTravelResume` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~OrbitalDockingDepartureTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Dialogue/DialogueEffectTransaction.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/RuntimeMotion.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/OrbitalDockingDepartureTests.cs"
...
### EP-0002-US-0005-TK-0001-orbital-synchronization
