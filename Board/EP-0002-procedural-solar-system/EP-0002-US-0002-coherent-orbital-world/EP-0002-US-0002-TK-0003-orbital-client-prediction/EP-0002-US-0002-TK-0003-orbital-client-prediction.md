...
ticket: EP-0002-US-0002-TK-0003-orbital-client-prediction
title: Плавный прогноз орбит из клиентского буфера
stage: approved
layer: client
depends_on: [EP-0002-US-0001-TK-0001-system-map-contract, EP-0002-US-0001-TK-0002-generation-input-schema, EP-0002-US-0001-TK-0003-seeded-world-bootstrap, EP-0002-US-0001-TK-0004-default-system-content, EP-0002-US-0001-TK-0005-initial-system-view, EP-0002-US-0002-TK-0002-authoritative-orbit-runtime]
files_touched: 3
...
# Плавный прогноз орбит из клиентского буфера

## Why

Согласованное движение орбитального мира: плавный прогноз орбит из клиентского буфера даёт проверяемый шаг к результату истории. End state: одинаковая эпоха, Speed0..4, точные фазы; screen позиции соответствуют Motion.; два кадра на паузе и первый после resume без орбитального скачка.
...
| src/DeepSpaceSaga.Client/UI/Screens/GameSession/RenderMotion.cs | src/DeepSpaceSaga.Client/UI/Screens/GameSession/RenderMotion.cs:1 — using DeepSpaceSaga.Contracts; | Только алгоритм/представление, явно названные в Implementation steps этого тикета; остальные функции не менять. |
| tests/DeepSpaceSaga.Client.Tests/OrbitalClientPredictionTests.cs | Новый файл; владелец EP-0002-US-0002-TK-0003-orbital-client-prediction. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change. Читаются Orbit, OrbitSampleSimulationTimeMs, WorldOffsetX/Y из ObjectMotionSnapshot; render loop использует только SnapshotBuffer.

## Implementation steps

1. Использовать общий IMotionPredictor для объектов Orbit, не линейный fast path и не календарный delta. При паузе prediction delta=0; возобновление не заменяет точную орбиту stale visual anchor.
2. Пристыкованный ship использует snapshot Orbit+world offset, не независимо экстраполированную касательную. Сравнить позиции screen.RenderStates и Motion oracle; сглаживание не меняет authoritative pose.
...
Test class: `OrbitalClientPredictionTests`. Тестовые сценарии и ожидаемые результаты:

- `OrbitalClientPredictionTests.ClientAndEngineOrbitAgree` — одинаковая эпоха, Speed0..4, точные фазы; screen позиции соответствуют Motion.
- `OrbitalClientPredictionTests.PauseResumeNoDrift` — два кадра на паузе и первый после resume без орбитального скачка.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `OrbitalClientPredictionTests.ClientAndEngineOrbitAgree`, `OrbitalClientPredictionTests.PauseResumeNoDrift` |
| AC-0002 | `OrbitalClientPredictionTests.ClientAndEngineOrbitAgree`, `OrbitalClientPredictionTests.PauseResumeNoDrift` |
| AC-0003 | `OrbitalClientPredictionTests.ClientAndEngineOrbitAgree`, `OrbitalClientPredictionTests.PauseResumeNoDrift` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~OrbitalClientPredictionTests"
...
### EP-0002-US-0002-TK-0002-authoritative-orbit-runtime
