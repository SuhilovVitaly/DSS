...
ticket: EP-0002-US-0002-TK-0002-authoritative-orbit-runtime
title: Орбиты и пристыкованные корабли в авторитетном мире
stage: approved
layer: engine
depends_on: [EP-0002-US-0001-TK-0001-system-map-contract, EP-0002-US-0001-TK-0002-generation-input-schema, EP-0002-US-0001-TK-0003-seeded-world-bootstrap, EP-0002-US-0001-TK-0004-default-system-content, EP-0002-US-0001-TK-0005-initial-system-view, EP-0002-US-0002-TK-0001-absolute-orbit-math]
files_touched: 5
...
# Орбиты и пристыкованные корабли в авторитетном мире

## Why

Согласованное движение орбитального мира: орбиты и пристыкованные корабли в авторитетном мире даёт проверяемый шаг к результату истории. End state: Speed0..4 и календарный +1h дают одинаковую позу при одинаковом simulationTime.; offset (1,1) сохраняется, свободный корабль 4 km/s за 1 physical s проходит 40 world.; дробное продвижение совпадает с прямым до одной эпохи.
...
| src/DeepSpaceSaga.Engine/SimulationEngine.EconomyTime.cs | src/DeepSpaceSaga.Engine/SimulationEngine.EconomyTime.cs:12 — private void AdvanceWorldTo(long gameTimeMs, long simulationTimeMs) | Только алгоритм/представление, явно названные в Implementation steps этого тикета; остальные функции не менять. |
| tests/DeepSpaceSaga.Engine.Tests/AuthoritativeOrbitRuntimeTests.cs | Новый файл; владелец EP-0002-US-0002-TK-0002-authoritative-orbit-runtime. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No new public API. Snapshot OrbitSampleSimulationTimeMs равен времени движения снимка; docking projection получает Orbit станции и WorldOffsetX/Y=(1,1), поля созданы EP-0002-US-0001-TK-0001. SpaceObjectRuntime хранит binding в InitialMotion; RuntimeMotion.At использует абсолютное время.

## Implementation steps

1. Провести Orbit от SpaceObjectData к InitialMotion/runtime, во все PredictMotion/RuntimeMotion.At и snapshot sample timestamp. Orbit нельзя пропускать через линейную оптимизацию BuildSnapshot. Орбитальные позы вычислять из _processedSimulationTimeMs/clock.MotionTimeMs, не GameTimeMs.
2. Пристыкованный корабль получает позу станции+(1,1), скорость и касательную станции через общий evaluator; parent binding в runtime хранить как resolved OrbitalElements+offset, исключить рекурсию. Команды корабля не изменяют parent orbit.
3. Проверить AdvanceWorldTo(gameTime,simulationTime), станционный +1h переход и Speed0–4; сначала вычислить конечные абсолютные орбиты, затем docked poses. Sun stationary; свободное движение и engine cycles не ускоряются календарным коэффициентом.
...
Test class: `AuthoritativeOrbitRuntimeTests`. Тестовые сценарии и ожидаемые результаты:

- `AuthoritativeOrbitRuntimeTests.AllSpeedsMatchMotion` — Speed0..4 и календарный +1h дают одинаковую позу при одинаковом simulationTime.
- `AuthoritativeOrbitRuntimeTests.DockedOffsetAndFreeFlight` — offset (1,1) сохраняется, свободный корабль 4 km/s за 1 physical s проходит 40 world.
- `AuthoritativeOrbitRuntimeTests.SplitAdvanceEqualsDirect` — дробное продвижение совпадает с прямым до одной эпохи.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `AuthoritativeOrbitRuntimeTests.AllSpeedsMatchMotion`, `AuthoritativeOrbitRuntimeTests.DockedOffsetAndFreeFlight`, `AuthoritativeOrbitRuntimeTests.SplitAdvanceEqualsDirect` |
| AC-0002 | `AuthoritativeOrbitRuntimeTests.AllSpeedsMatchMotion`, `AuthoritativeOrbitRuntimeTests.DockedOffsetAndFreeFlight`, `AuthoritativeOrbitRuntimeTests.SplitAdvanceEqualsDirect` |
| AC-0003 | `AuthoritativeOrbitRuntimeTests.AllSpeedsMatchMotion`, `AuthoritativeOrbitRuntimeTests.DockedOffsetAndFreeFlight`, `AuthoritativeOrbitRuntimeTests.SplitAdvanceEqualsDirect` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~AuthoritativeOrbitRuntimeTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/RuntimeMotion.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.SolarSystem.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.EconomyTime.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/AuthoritativeOrbitRuntimeTests.cs"
...
### EP-0002-US-0002-TK-0001-absolute-orbit-math
