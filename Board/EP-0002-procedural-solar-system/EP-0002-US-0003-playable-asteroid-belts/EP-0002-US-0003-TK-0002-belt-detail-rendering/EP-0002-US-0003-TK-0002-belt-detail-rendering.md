...
ticket: EP-0002-US-0003-TK-0002-belt-detail-rendering
title: Неоднородные пояса и выбор игровых астероидов
stage: approved
layer: client
depends_on: [EP-0002-US-0002-TK-0001-absolute-orbit-math, EP-0002-US-0002-TK-0002-authoritative-orbit-runtime, EP-0002-US-0002-TK-0003-orbital-client-prediction, EP-0002-US-0003-TK-0001-seeded-belt-asteroids]
files_touched: 4
...
# Неоднородные пояса и выбор игровых астероидов

## Why

Наполненные астероидные пояса: неоднородные пояса и выбор игровых астероидов даёт проверяемый шаг к результату истории. End state: на system видны границы, на local выбираются реальные астероиды.; cache rebuild на новый map, стабильность между кадрами/zoom, число snapshot objects неизменно.
...
| tests/DeepSpaceSaga.Client.Tests/BeltDetailRenderingTests.cs | Новый файл; владелец EP-0002-US-0003-TK-0002-belt-detail-rendering. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change. Рисунок только из snapshot belt bounds+decoration seed.

## Implementation steps

1. Кэшировать рисунок по belt ID/DecorationSeed/уровню детализации, с разрывами и локальными сгущениями; cull viewport. Не строить тысячи ObjectMotionSnapshot для рисунка.
2. При приближении раскрывать Permanent asteroid markers и hit testing; декоративные точки не выбираются и не закрывают игровые астероиды. System-fit включает весь annulus.
...
Test class: `BeltDetailRenderingTests`. Тестовые сценарии и ожидаемые результаты:

- `BeltDetailRenderingTests.BeltDetailLodAndSelection` — на system видны границы, на local выбираются реальные астероиды.
- `BeltDetailRenderingTests.DensityKeepsEntityCount` — cache rebuild на новый map, стабильность между кадрами/zoom, число snapshot objects неизменно.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `BeltDetailRenderingTests.BeltDetailLodAndSelection`, `BeltDetailRenderingTests.DensityKeepsEntityCount` |
| AC-0002 | `BeltDetailRenderingTests.BeltDetailLodAndSelection`, `BeltDetailRenderingTests.DensityKeepsEntityCount` |
| AC-0003 | `BeltDetailRenderingTests.BeltDetailLodAndSelection`, `BeltDetailRenderingTests.DensityKeepsEntityCount` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~BeltDetailRenderingTests"
...
### EP-0002-US-0003-TK-0001-seeded-belt-asteroids
