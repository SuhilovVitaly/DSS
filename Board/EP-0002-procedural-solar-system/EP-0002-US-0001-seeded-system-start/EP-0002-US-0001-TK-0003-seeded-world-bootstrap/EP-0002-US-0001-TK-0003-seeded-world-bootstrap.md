...
ticket: EP-0002-US-0001-TK-0003-seeded-world-bootstrap
title: Атомарная генерация Default с масштабом 50–75 дней
stage: approved
layer: engine
depends_on: [EP-0002-US-0001-TK-0002-generation-input-schema]
files_touched: 5
...
# Атомарная генерация Default с масштабом 50–75 дней

## Why

Новая игра в воспроизводимой Солнечной системе: атомарная генерация default с масштабом 50–75 дней даёт проверяемый шаг к результату истории. End state: два запуска, seeds 1 и 2, стабильные ID/типы/геометрия; другой seed меняет fingerprint.; независимое вычисление для 50/75, Vmax 4 и 2, скорости старта 0/0.7, docking (1,1).; невозможный clearance, collisions IDs: конечный отказ без нового snapshot/live-state mutation.
...
| tests/DeepSpaceSaga.Engine.Tests/SeededWorldBootstrapTests.cs | Новый файл; владелец EP-0002-US-0001-TK-0003-seeded-world-bootstrap. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Реализовать Generate в detached ScenarioFile до формирования runtimeObjects. Использовать resolved master seed до первого draw: RngStreamSeedDerivation.DeriveStreamSeed(seed,"solar-system/v1/{stage}/{attempt}") и фиксированный SplitMix64 (state+=0x9E3779B97F4A7C15; xor>>30,*0xBF58476D1CE4E5B9; xor>>27,*0x94D049BB133111EB; xor>>31), unsigned unchecked; double=(next>>11)*2^-53. Все обходы ID ordinal.
2. Определить Vmax по установленному работоспособному Engine модулю стартового корабля; нет конечного положительного Vmax — ошибка. Выбрать дни [50,75], radius=Vmax*days*86400/300*10. Единым переносом Default сохранить локальные координатные разности и docking offset; радиус проверять по кораблю после переноса.
3. Создать одно Sun в (0,0), 3–7 планет Rocky/Icy/Gas и 2–5 непересекающихся радиальных поясов. Сначала зарезервировать пояс стартовой группы, далее отсортированные непересекающиеся коридоры с clearance. Генерировать circular орбиты с периодом 2*pi*r/(Vmax*OrbitSpeedFraction*10)*1000*300, округление long checked; сохранить фазы/эпохи. ID SYS-SUN, SYS-PLANET-{ordinal}, SYS-BELT-{ordinal}; коллизия со сценарием — ошибка, не перенумерация.
4. SystemRadius=1.1*max(apocenter,outer belt radius,existing object extent). После MaxPlacementAttempts возвращать ScenarioException с seed/version/stage/attempt/reason. Load и direct unit-fixture LoadScenario без config не генерируют. Публиковать map и полные известные render metadata; вызвать только одну pipeline.
5. В EngineContentLoader подключить reader TK-0002 только к двум New Game entrypoints. enabledScenarios сопоставлять строго с именем родительской папки scenario.json (Default/Default_500/Docked/Undocked/MarketProfiles), независимо от display name. Fail не заменяет ранее работающий engine state.
...
Test class: `SeededWorldBootstrapTests`. Тестовые сценарии и ожидаемые результаты:

- `SeededWorldBootstrapTests.SameSeedSameWorld` — два запуска, seeds 1 и 2, стабильные ID/типы/геометрия; другой seed меняет fingerprint.
- `SeededWorldBootstrapTests.ActualPlayerStartDays` — независимое вычисление для 50/75, Vmax 4 и 2, скорости старта 0/0.7, docking (1,1).
- `SeededWorldBootstrapTests.ExhaustedPlacementIsAtomic` — невозможный clearance, collisions IDs: конечный отказ без нового snapshot/live-state mutation.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `SeededWorldBootstrapTests.SameSeedSameWorld`, `SeededWorldBootstrapTests.ActualPlayerStartDays`, `SeededWorldBootstrapTests.ExhaustedPlacementIsAtomic` |
| AC-0002 | `SeededWorldBootstrapTests.SameSeedSameWorld`, `SeededWorldBootstrapTests.ActualPlayerStartDays`, `SeededWorldBootstrapTests.ExhaustedPlacementIsAtomic` |
| AC-0003 | `SeededWorldBootstrapTests.SameSeedSameWorld`, `SeededWorldBootstrapTests.ActualPlayerStartDays`, `SeededWorldBootstrapTests.ExhaustedPlacementIsAtomic` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~SeededWorldBootstrapTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.SolarSystem.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Content/EngineContentLoader.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/SeededWorldBootstrapTests.cs"
