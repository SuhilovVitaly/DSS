...
ticket: EP-0002-US-0001-TK-0001-system-map-contract
title: Сериализуемая структура системы и точные орбитальные данные
stage: approved
layer: contracts
depends_on: []
files_touched: 4
...
# Сериализуемая структура системы и точные орбитальные данные

## Why

Новая игра в воспроизводимой Солнечной системе: сериализуемая структура системы и точные орбитальные данные даёт проверяемый шаг к результату истории. End state: старый JSON не требует новых полей; default arrays не ломают сериализацию.; 17° + 0.000001° сохраняются без округления, ulong seed и эпоха сохраняются.
...
|---|---|---|
| src/DeepSpaceSaga.Contracts/SolarSystemMap.cs | Новый файл; владелец EP-0002-US-0001-TK-0001-system-map-contract. В исходном дереве отсутствует. | DTO системы/поясов/планет/точных орбит и их explicit JSON properties. |
| src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs | src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs:11 — public sealed record AuthoritativeSnapshot( | Только optional map block этого тикета, immutable JSON-safe boundary. |
| src/DeepSpaceSaga.Contracts/ObjectMotionSnapshot.cs | src/DeepSpaceSaga.Contracts/ObjectMotionSnapshot.cs:11 — public sealed record ObjectMotionSnapshot( | Только описанные optional motion/offset/sample metadata; прежние positional arguments совместимы. |
| tests/DeepSpaceSaga.Contracts.Tests/SystemMapContractTests.cs | Новый файл; владелец EP-0002-US-0001-TK-0001-system-map-contract. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить immutable DTO системы, поясов, планет и орбит; optional SolarSystemMap на AuthoritativeSnapshot и optional Orbit/OrbitSampleSimulationTimeMs на ObjectMotionSnapshot. Старые конструкторы и JSON без новых полей продолжают работать.
2. Зафиксировать world units, календарный период и epoch времени движения; legacy initialPhase остаётся целым, PhaseOffsetDegrees добавляет точность. Коллекции default нормализуются в пустые, только сериализуемые типы.
...
Test class: `SystemMapContractTests`. Тестовые сценарии и ожидаемые результаты:

- `SystemMapContractTests.LegacySnapshotRoundTrip` — старый JSON не требует новых полей; default arrays не ломают сериализацию.
- `SystemMapContractTests.PrecisePhaseRoundTrip` — 17° + 0.000001° сохраняются без округления, ulong seed и эпоха сохраняются.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `SystemMapContractTests.LegacySnapshotRoundTrip`, `SystemMapContractTests.PrecisePhaseRoundTrip` |
| AC-0002 | `SystemMapContractTests.LegacySnapshotRoundTrip`, `SystemMapContractTests.PrecisePhaseRoundTrip` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --no-restore --filter "FullyQualifiedName~SystemMapContractTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/DeepSpaceSaga.Contracts.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/SolarSystemMap.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/ObjectMotionSnapshot.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/SystemMapContractTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/SolarSystemMap.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/ObjectMotionSnapshot.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/SystemMapContractTests.cs"
...
none. Используется текущий API из Code context.
