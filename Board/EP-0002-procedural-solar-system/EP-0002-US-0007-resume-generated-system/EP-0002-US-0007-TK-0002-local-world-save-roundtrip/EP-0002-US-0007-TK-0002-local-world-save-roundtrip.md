...
ticket: EP-0002-US-0007-TK-0002-local-world-save-roundtrip
title: Продолжение системы через локальный транспорт сохранений
stage: approved
layer: local-client
depends_on: [EP-0002-US-0005-TK-0001-orbital-synchronization, EP-0002-US-0005-TK-0002-orbital-docking-departure, EP-0002-US-0006-TK-0001-known-map-projection, EP-0002-US-0006-TK-0002-system-map-navigation, EP-0002-US-0007-TK-0001-generated-world-persistence]
files_touched: 2
...
# Продолжение системы через локальный транспорт сохранений

## Why

Продолжение той же системы после загрузки: продолжение системы через локальный транспорт сохранений даёт проверяемый шаг к результату истории. End state: пять сценариев, orbit/seed/IDs и next snapshot.; invalid version/parent, файл побайтно прежний.
...
| tests/DeepSpaceSaga.Engine.Tests/LocalWorldSaveRoundtripTests.cs | Новый файл; владелец EP-0002-US-0007-TK-0002-local-world-save-roundtrip. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change. Используются существующие public factory/save/load contracts; matching test project — DeepSpaceSaga.Engine.Tests, имеющий reference на LocalClient.
...
2. Добавить integration tests реального LocalGameSessionConnection save/load через временный каталог, Settings и выбранный сценарий; не изменять Engine/Client production.
3. Зафиксировать snapshot перед pause/save, закрыть сессию и открыть save; затем продвинуть обе контрольные сессии одинаково. Проверить legacy save и failed load: прежний файл не перезаписан, частичная сессия не выдана.
...
Test class: `LocalWorldSaveRoundtripTests`. Тестовые сценарии и ожидаемые результаты:

- `LocalWorldSaveRoundtripTests.LocalSaveRestoresSameSystem` — пять сценариев, orbit/seed/IDs и next snapshot.
- `LocalWorldSaveRoundtripTests.FailedLoadLeavesSaveUntouched` — invalid version/parent, файл побайтно прежний.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `LocalWorldSaveRoundtripTests.LocalSaveRestoresSameSystem`, `LocalWorldSaveRoundtripTests.FailedLoadLeavesSaveUntouched` |
| AC-0002 | `LocalWorldSaveRoundtripTests.LocalSaveRestoresSameSystem`, `LocalWorldSaveRoundtripTests.FailedLoadLeavesSaveUntouched` |
| AC-0003 | `LocalWorldSaveRoundtripTests.LocalSaveRestoresSameSystem`, `LocalWorldSaveRoundtripTests.FailedLoadLeavesSaveUntouched` |
...
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~LocalWorldSaveRoundtripTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine.LocalClient/DeepSpaceSaga.Engine.LocalClient.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/LocalWorldSaveRoundtripTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/LocalWorldSaveRoundtripTests.cs"
...
### EP-0002-US-0007-TK-0001-generated-world-persistence
