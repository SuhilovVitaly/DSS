...
ticket: EP-0002-US-0004-TK-0001-scenario-group-translation
title: Перенос сценарных групп с сохранением связей
stage: approved
layer: engine
depends_on: [EP-0002-US-0003-TK-0001-seeded-belt-asteroids, EP-0002-US-0003-TK-0002-belt-detail-rendering]
files_touched: 4
...
# Перенос сценарных групп с сохранением связей

## Why

Привычные сценарии в новом масштабе: перенос сценарных групп с сохранением связей даёт проверяемый шаг к результату истории. End state: Default/Default_500/Docked/Undocked/MarketProfiles сохраняют пары расстояний, экипаж, модули, ресурсы, inventory.; точные 50/75 дней по игроку, Docked/Undocked state.; новый scenario.json обнаруживается тестом и требует явной поддержки.
...
| src/DeepSpaceSaga.Engine/Scenario/ScenarioGroupPlacement.cs | Новый файл; владелец EP-0002-US-0004-TK-0001-scenario-group-translation. В исходном дереве отсутствует. | Только алгоритм/представление, явно названные в Implementation steps этого тикета; остальные функции не менять. |
...
| tests/DeepSpaceSaga.Engine.Tests/ScenarioGroupTranslationTests.cs | Новый файл; владелец EP-0002-US-0004-TK-0001-scenario-group-translation. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Вынести единый rigid translation для всех исходных объектов, включая player, stations и Default_500 окружение; относительные расстояния/ID/данные не менять. Орбитальную станцию разместить по итоговой позиции, другие исходные группы сохранить как перевод исходной мировой геометрии.
2. После docking normalization вычислить Vmax и фактический radius; подбирать translation по player конечной позиции. Не клонировать станции, не запускать fallback поверх explicit inventory. Отсутствующий Vmax/ссылка — fail before publication.
3. Различать production New Game и fixtures/Save: включение только bootstrap Settings и enabled scenario names; явно заданный masterSeed воспроизводим. Тест перечисляет все реальные Scenarios/**/scenario.json и требует присутствия каждого игрового сценария в enabledScenarios после content тикета.
...
Test class: `ScenarioGroupTranslationTests`. Тестовые сценарии и ожидаемые результаты:

- `ScenarioGroupTranslationTests.FiveScenarioIdentityMatrix` — Default/Default_500/Docked/Undocked/MarketProfiles сохраняют пары расстояний, экипаж, модули, ресурсы, inventory.
- `ScenarioGroupTranslationTests.DockingOffsetAtRadiusBoundaries` — точные 50/75 дней по игроку, Docked/Undocked state.
- `ScenarioGroupTranslationTests.AllShippedScenariosCovered` — новый scenario.json обнаруживается тестом и требует явной поддержки.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `ScenarioGroupTranslationTests.FiveScenarioIdentityMatrix`, `ScenarioGroupTranslationTests.DockingOffsetAtRadiusBoundaries`, `ScenarioGroupTranslationTests.AllShippedScenariosCovered` |
| AC-0002 | `ScenarioGroupTranslationTests.FiveScenarioIdentityMatrix`, `ScenarioGroupTranslationTests.DockingOffsetAtRadiusBoundaries`, `ScenarioGroupTranslationTests.AllShippedScenariosCovered` |
| AC-0003 | `ScenarioGroupTranslationTests.FiveScenarioIdentityMatrix`, `ScenarioGroupTranslationTests.DockingOffsetAtRadiusBoundaries`, `ScenarioGroupTranslationTests.AllShippedScenariosCovered` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~ScenarioGroupTranslationTests"
