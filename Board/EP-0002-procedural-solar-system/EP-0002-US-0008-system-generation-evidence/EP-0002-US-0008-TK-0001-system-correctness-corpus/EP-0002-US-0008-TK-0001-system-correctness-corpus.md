...
ticket: EP-0002-US-0008-TK-0001-system-correctness-corpus
title: Воспроизводимый корпус генерации по 100 seed
stage: approved
layer: engine
depends_on: [EP-0002-US-0007-TK-0001-generated-world-persistence, EP-0002-US-0007-TK-0002-local-world-save-roundtrip]
files_touched: 1
...
# Воспроизводимый корпус генерации по 100 seed

## Why

Проверяемая устойчивость генерации системы: воспроизводимый корпус генерации по 100 seed даёт проверяемый шаг к результату истории. End state: все комбинации с replay строки при failure.; конечный atomic отказ и сохранение прежнего live-state.
...
|---|---|---|
| tests/DeepSpaceSaga.Engine.Tests/SystemCorrectnessCorpusTests.cs | Новый файл; владелец EP-0002-US-0008-TK-0001-system-correctness-corpus. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Реальный bootstrap всех пяти сценариев для seeds 1..100; для каждого — min/max 3/7 планет и 2/5 поясов, radius 50/75, pause/all speeds, save continuation. Независимые формулы проверяют scale/коридоры, а не fingerprint генератора против самого себя.
2. Добавить invalid catalog/type/ID/config и исчерпание attempts; отчёт failures содержит seed/scenario/config/generator version/criterion. Тесты производительности не смешивать с assertions корректности.
...
Test class: `SystemCorrectnessCorpusTests`. Тестовые сценарии и ожидаемые результаты:

- `SystemCorrectnessCorpusTests.HundredSeedScenarioBoundaryCorpus` — все комбинации с replay строки при failure.
- `SystemCorrectnessCorpusTests.InvalidGenerationCorpus` — конечный atomic отказ и сохранение прежнего live-state.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `SystemCorrectnessCorpusTests.HundredSeedScenarioBoundaryCorpus`, `SystemCorrectnessCorpusTests.InvalidGenerationCorpus` |
| AC-0003 | `SystemCorrectnessCorpusTests.HundredSeedScenarioBoundaryCorpus`, `SystemCorrectnessCorpusTests.InvalidGenerationCorpus` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~SystemCorrectnessCorpusTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/SystemCorrectnessCorpusTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/SystemCorrectnessCorpusTests.cs"
