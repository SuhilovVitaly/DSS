...
ticket: EP-0005-US-0002-TK-0001-frame-state-update
title: "Выделить обновление состояния тактического кадра"
stage: draft
layer: client
...
Сейчас Render обновляет presentation state, выполняет reconciliation и готовит объекты для рисования. Это затрудняет повтор кадра и независимую проверку состояния.
...
Целевой результат: Выделить обновление состояния тактического кадра. Значения новых порогов, явно названные draft assumption, подлежат согласованию при утверждении тикета.
...
- `src/DeepSpaceSaga.Client/UI/Screens/GameSession/TacticalMapFrameState.cs` — создать в этом тикете / после зависимости
- `src/DeepSpaceSaga.Client/UI/Screens/GameSession/TacticalMapStateUpdater.cs` — создать в этом тикете / после зависимости
- `tests/DeepSpaceSaga.Client.Tests/TacticalMapFrameStateTests.cs` — создать в этом тикете / после зависимости
...
- EP-0005-US-0001-tactical-map-audit-remediation

## Публичный API

План: TacticalMapStateUpdater.Update(SnapshotPrediction? prediction, long timestamp, double deltaSeconds) -> TacticalMapFrameState. Типы client-internal.

## Порядок реализации

1. Выделить Update с одним snapshot baseline и одним monotonic timestamp на кадр: prediction, reconciliation, pause/rebase, contacts и camera follow.
2. Вернуть immutable TacticalMapFrameState с frame ID, snapshot revision, MotionTimeMs и rendered poses; не отдавать изменяемые scratch buffers потребителям.
3. Сохранить существующие integration tests и поведение F02/R03. На этом шаге адаптер вызывает прежний renderer с новым состоянием. Ни Skia, ни filesystem, ни session commands в updater.
...
1. Добавить/обновить именованные проверки выше в `tests/DeepSpaceSaga.Client.Tests/TacticalMapFrameStateTests.cs`. Проверять наблюдаемое поведение, а не копию реализации.
...
4. `dotnet format whitespace DeepSpaceSaga.sln --verify-no-changes --no-restore --include src/DeepSpaceSaga.Client/UI/Screens/GameSession/GameSessionScreen.cs src/DeepSpaceSaga.Client/UI/Screens/GameSession/TacticalMapFrameState.cs src/DeepSpaceSaga.Client/UI/Screens/GameSession/TacticalMapStateUpdater.cs tests/DeepSpaceSaga.Client.Tests/TacticalMapFrameStateTests.cs` — запускать из solution directory; при нескольких solutions явно указать актуальную `.sln`/`.slnx`.
