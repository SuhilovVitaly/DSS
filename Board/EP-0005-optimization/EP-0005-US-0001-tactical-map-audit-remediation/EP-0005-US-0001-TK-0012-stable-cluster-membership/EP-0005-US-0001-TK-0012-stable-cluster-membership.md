...
ticket: EP-0005-US-0001-TK-0012-stable-cluster-membership
title: "Проверить устойчивость кластеров при плавном zoom"
stage: draft
layer: client
depends_on: ["EP-0005-US-0001-TK-0003-cluster-click-priority"]
files_touched: 3
serves: [AC-0012]
source_finding: R01
...
# EP-0005-US-0001-TK-0012-stable-cluster-membership

## Зачем

Риск: Floor(world/cellSize) при continuous zoom часто меняет membership. Масштаб эффекта визуально не измерен.
...
Целевой результат: Проверить устойчивость кластеров при плавном zoom. Значения новых порогов, явно названные draft assumption, подлежат согласованию при утверждении тикета.
...
- `src/DeepSpaceSaga.Client/UI/TacticalMapSettings.cs` — существует
- `tests/DeepSpaceSaga.Client.Tests/TacticalMapClusterStabilityTests.cs` — создать в этом тикете / после зависимости
...
1. Воспроизвести pan/zoom около cell boundaries на fixed contacts; записать membership changes.
2. При подтверждении использовать дискретные world levels с hysteresis входа/выхода и stable cluster IDs; pan не меняет membership.
3. Стабильный centroid/bounds/order, important contacts исключены. Если риск не воспроизводится, оставить regression/evidence без смены поведения.

## Критерии приёмки — AC-0012

- [ ] `Cluster_membership_is_stable_within_zoom_band`: Микроколебания zoom внутри уровня не меняют состав.
- [ ] `Pan_does_not_reassign_cluster_cells`: Pan не меняет identity clusters.
- [ ] `Cluster_boundary_transition_is_deterministic`: Есть reproduce/fix или обоснованный not-reproduced outcome.
...
1. Добавить/обновить именованные проверки выше в `tests/DeepSpaceSaga.Client.Tests/TacticalMapClusterStabilityTests.cs`. Проверять наблюдаемое поведение, а не копию реализации.
