...
# Стартовый кластер из экономических ролей

## Why

Рабочая торговля в стартовом кластере: стартовый кластер из экономических ролей даёт проверяемый шаг к результату истории. End state: пять сценариев, 10/12, исходные ID/inventory/roles остаются.; independent graph algorithm, producers/consumers и transit exception.; попарные расстояния до/после, движение, фактический старт 50–75d.; исходные 200 world между mining/industrial не превращаются в5760 world, generated neighbour links соблюдают диапазон.
...
| tests/DeepSpaceSaga.Engine.Tests/LocalClusterGenerationTests.cs | Новый файл; владелец EP-0003-US-0001-TK-0002-local-cluster-generation. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Расширить конфигурацию optional Clusters. Сначала строить роли пяти профилей/потоки supply→demand по registry и supply/demand semantics EP-0001; новый helper внутри StationClusterGenerator строит граф для текущих10–12 members и выдаёт перечисленные ClusterTradeLink. Fixed-five validator/placement EP-0001 не вызывать для этого режима и не изменять. Геометрия использует результат graph helper. Сценарные станции входят в quota, их ID/explicit данные сохраняются.
2. Создать 10–12 станций со всеми пятью market.* профилями, транзит без обязательного производства. Для каждого producer основной output имеет consumer; граф strongly connected, >=2 различных исходящих соседа и cycle rank E-V+1>=2 для underlying undirected graph. Стартовое ребро имеет товарный поток для обратной загрузки.
3. Сохранить исходную сценарную группу как жёсткую формацию с общей angular velocity, включая радиальные offsets исходных станций; новые станции размещать на короткой дуге. Такая формация сохраняет все исходные попарные расстояния (не принуждать неколлинеарные исходные станции к одному радиусу). Вся формация в ширине пояса; nearest trading neighbours 0.5–2d, diameter 3–7d по Vmax.
4. Встроить stage после базовых поясов до runtime publication, установить market profiles через existing ResolveMarketProfiles и ClusterMap projection. Не трогать explicit inventory и radius 50–75d после окончательной docking normalization. На невозможной форме — bounded retry/error.
5. Исключение начального balancing диапазона: все исходные station-station расстояния MarketProfiles сохраняются, даже если короче0.5d; их связи помечаются preserved-scenario в диагностике. NeighbourMin/Max применяется к новым генерируемым связям, diameter — ко всей группе. Не растягивать исходные 5 станций ради параметра. Отдельный named test PreservedScenarioNeighbourException проверяет это исключение.
6. Добавить именованные тесты ниже, привязать результаты к served criteria и записать фактические команды/результат в review evidence этого тикета. Нельзя помечать passed ещё не выполненный прогон.
...
Test class: `LocalClusterGenerationTests`. Тестовые сценарии и ожидаемые результаты:

- `LocalClusterGenerationTests.ScenarioStationsCountTowardQuota` — пять сценариев, 10/12, исходные ID/inventory/roles остаются.
- `LocalClusterGenerationTests.LocalGraphHasTwoCyclesAndReturnCargo` — independent graph algorithm, producers/consumers и transit exception.
- `LocalClusterGenerationTests.RigidScenarioDistancesAndStartRadius` — попарные расстояния до/после, движение, фактический старт 50–75d.
- `LocalClusterGenerationTests.PreservedScenarioNeighbourException` — исходные 200 world между mining/industrial не превращаются в5760 world, generated neighbour links соблюдают диапазон.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `LocalClusterGenerationTests.ScenarioStationsCountTowardQuota`, `LocalClusterGenerationTests.LocalGraphHasTwoCyclesAndReturnCargo`, `LocalClusterGenerationTests.RigidScenarioDistancesAndStartRadius`, `LocalClusterGenerationTests.PreservedScenarioNeighbourException` |
| AC-0002 | `LocalClusterGenerationTests.ScenarioStationsCountTowardQuota`, `LocalClusterGenerationTests.LocalGraphHasTwoCyclesAndReturnCargo`, `LocalClusterGenerationTests.RigidScenarioDistancesAndStartRadius`, `LocalClusterGenerationTests.PreservedScenarioNeighbourException` |
| AC-0003 | `LocalClusterGenerationTests.ScenarioStationsCountTowardQuota`, `LocalClusterGenerationTests.LocalGraphHasTwoCyclesAndReturnCargo`, `LocalClusterGenerationTests.RigidScenarioDistancesAndStartRadius`, `LocalClusterGenerationTests.PreservedScenarioNeighbourException` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~LocalClusterGenerationTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/StationClusterGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGeneration.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/LocalClusterGenerationTests.cs"
