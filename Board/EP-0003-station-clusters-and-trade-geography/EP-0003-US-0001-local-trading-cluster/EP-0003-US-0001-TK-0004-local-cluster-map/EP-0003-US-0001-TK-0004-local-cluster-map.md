...
# Стартовый район и доступные соседние станции

## Why

Рабочая торговля в стартовом кластере: стартовый район и доступные соседние станции даёт проверяемый шаг к результату истории. End state: 10/12 станций, selection из system/local view.; профиль не превращает объект в AI.; экран/цена доступны по прежнему dock contract.
...
| tests/DeepSpaceSaga.Client.Tests/LocalClusterMapTests.cs | Новый файл; владелец EP-0003-US-0001-TK-0004-local-cluster-map. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No session API change. internal ClusterMapPresentation — чистая проекция ClusterMap+snapshot; ObjectInfoPanelData получает optional cluster/profile strings.

## Implementation steps

1. Показывать название стартового кластера и пять профилей из ClusterMap, сохранять отдельный выбор каждой станции; Screen.MapCluster остаётся только визуальной агрегацией.
2. Карточка станции показывает cluster/profile и потенциальные товарные направления без remote quote; переход к существующему station/trade доступен только при authoritative dock. Не вводить второй Trade screen.
...
Test class: `LocalClusterMapTests`. Тестовые сценарии и ожидаемые результаты:

- `LocalClusterMapTests.StartClusterShowsEveryStation` — 10/12 станций, selection из system/local view.
- `LocalClusterMapTests.HumanScientificMilitaryIsOrdinaryMarket` — профиль не превращает объект в AI.
- `LocalClusterMapTests.RemoteSelectionDoesNotOpenTrade` — экран/цена доступны по прежнему dock contract.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `LocalClusterMapTests.StartClusterShowsEveryStation`, `LocalClusterMapTests.HumanScientificMilitaryIsOrdinaryMarket`, `LocalClusterMapTests.RemoteSelectionDoesNotOpenTrade` |
| AC-0002 | `LocalClusterMapTests.StartClusterShowsEveryStation`, `LocalClusterMapTests.HumanScientificMilitaryIsOrdinaryMarket`, `LocalClusterMapTests.RemoteSelectionDoesNotOpenTrade` |
| AC-0003 | `LocalClusterMapTests.StartClusterShowsEveryStation`, `LocalClusterMapTests.HumanScientificMilitaryIsOrdinaryMarket`, `LocalClusterMapTests.RemoteSelectionDoesNotOpenTrade` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~LocalClusterMapTests"
...
### EP-0003-US-0001-TK-0003-local-cluster-content
