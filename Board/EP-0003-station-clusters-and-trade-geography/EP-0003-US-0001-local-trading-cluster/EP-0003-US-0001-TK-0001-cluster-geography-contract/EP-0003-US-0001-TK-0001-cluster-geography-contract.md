...
# Доменные кластеры и экономические связи карты

## Why

Рабочая торговля в стартовом кластере: доменные кластеры и экономические связи карты даёт проверяемый шаг к результату истории. End state: сохраняются состав/IDs/профили/связи, default collections.; старые constructors и JSON совместимы, ETA/quotes отсутствуют.
...
| tests/DeepSpaceSaga.Contracts.Tests/ClusterGeographyContractTests.cs | Новый файл; владелец EP-0003-US-0001-TK-0001-cluster-geography-contract. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить immutable snapshot кластеров со стабильными ID, поясом, названием, составом и профилями станций; membership не зависит от zoom. Связи хранят endpoints и cargo role, не вечный ETA или котировку.
2. Новый optional ClusterMap не требует полей от legacy snapshot. Профили — ссылки на existing market IDs, Human membership не выводится из scientific-military профиля.
...
Test class: `ClusterGeographyContractTests`. Тестовые сценарии и ожидаемые результаты:

- `ClusterGeographyContractTests.ClusterSnapshotRoundTrip` — сохраняются состав/IDs/профили/связи, default collections.
- `ClusterGeographyContractTests.LegacySnapshotHasNoCluster` — старые constructors и JSON совместимы, ETA/quotes отсутствуют.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `ClusterGeographyContractTests.ClusterSnapshotRoundTrip`, `ClusterGeographyContractTests.LegacySnapshotHasNoCluster` |
| AC-0002 | `ClusterGeographyContractTests.ClusterSnapshotRoundTrip`, `ClusterGeographyContractTests.LegacySnapshotHasNoCluster` |
| AC-0003 | `ClusterGeographyContractTests.ClusterSnapshotRoundTrip`, `ClusterGeographyContractTests.LegacySnapshotHasNoCluster` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --no-restore --filter "FullyQualifiedName~ClusterGeographyContractTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/DeepSpaceSaga.Contracts.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/StationClusterMap.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/ClusterGeographyContractTests.cs"
