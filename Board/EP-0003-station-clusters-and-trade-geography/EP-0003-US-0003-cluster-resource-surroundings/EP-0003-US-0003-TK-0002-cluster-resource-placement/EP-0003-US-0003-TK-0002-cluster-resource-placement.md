...
# Ресурсы по ролям без дублирования полей

## Why

Ресурсное окружение торговых районов: ресурсы по ролям без дублирования полей даёт проверяемый шаг к результату истории. End state: все пять профилей и допустимые composition/item IDs.; same seed, 365d, group offsets; старые field IDs сохранены.; equal elapsed control, никакого дополнительного прихода склада.
...
| src/DeepSpaceSaga.Engine/Scenario/StationResourceFields.cs | Плановый файл зависимости EP-0001-US-0005-TK-0002-seeded-resource-fields; пока не подтверждён runtime. Контракт dependency приведён ниже. | Адаптация canonical resource-field geometry/anchor к кластерному режиму; единственный source состава. |
...
| tests/DeepSpaceSaga.Engine.Tests/ClusterResourcePlacementTests.cs | Новый файл; владелец EP-0003-US-0003-TK-0002-cluster-resource-placement. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Использовать StationResourceFieldsState и конфигурацию EP-0001/US-0005 как единственного владельца состава и fieldId. Применить его roles по marketProfileId к каждому cluster; исходные поля переиспользовать, не генерировать второй набор.
2. Перевести station-local размещение в rotating group offsets через ClusterResourceBinding; каждый игровой resource asteroid получает ту же angular velocity/смещение. Сохранить composition, mass и catalog item IDs.
3. Публиковать известную пространственную геометрию/состав через существующий ресурсный snapshot; строгий unknown item/reference отказ до публикации. Поля не меняют station inventory при time advance; сравнить с контрольным миром без влияния поля.
...
Test class: `ClusterResourcePlacementTests`. Тестовые сценарии и ожидаемые результаты:

- `ClusterResourcePlacementTests.ClusterResourcesMatchEconomicRoles` — все пять профилей и допустимые composition/item IDs.
- `ClusterResourcePlacementTests.ResourcesRotateAndReplay` — same seed, 365d, group offsets; старые field IDs сохранены.
- `ClusterResourcePlacementTests.FieldsDoNotSupplyMarkets` — equal elapsed control, никакого дополнительного прихода склада.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `ClusterResourcePlacementTests.ClusterResourcesMatchEconomicRoles`, `ClusterResourcePlacementTests.ResourcesRotateAndReplay`, `ClusterResourcePlacementTests.FieldsDoNotSupplyMarkets` |
| AC-0002 | `ClusterResourcePlacementTests.ClusterResourcesMatchEconomicRoles`, `ClusterResourcePlacementTests.ResourcesRotateAndReplay`, `ClusterResourcePlacementTests.FieldsDoNotSupplyMarkets` |
| AC-0003 | `ClusterResourcePlacementTests.ClusterResourcesMatchEconomicRoles`, `ClusterResourcePlacementTests.ResourcesRotateAndReplay`, `ClusterResourcePlacementTests.FieldsDoNotSupplyMarkets` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~ClusterResourcePlacementTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/StationClusterGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/StationResourceFields.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/ClusterResourcePlacementTests.cs"
