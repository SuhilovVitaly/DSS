...
# Действующий местный и межкластерный рейс

## Why

Местный цикл и дальний рейс с возвращением: действующий местный и межкластерный рейс даёт проверяемый шаг к результату истории. End state: две остановки и обратный груз с matching authoritative receipts.; 2 разных cluster IDs, возврат в исходный, finite budget.; нет speed multiplier, нет двойной оплаты, actual time отдельно от estimate.
...
|---|---|---|
| src/DeepSpaceSaga.Engine/SimulationEngine.ClusterVoyages.cs | Новый файл; владелец EP-0003-US-0005-TK-0001-cluster-voyage-integration. В исходном дереве отсутствует. | Только алгоритм/представление, явно названные в Implementation steps этого тикета; остальные функции не менять. |
...
| tests/DeepSpaceSaga.Engine.Tests/ClusterVoyageIntegrationTests.cs | Новый файл; владелец EP-0003-US-0005-TK-0001-cluster-voyage-integration. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Адаптировать только географическую привязку действующего voyage lifecycle EP-0001: origin/destination route IDs разрешаются через ClusterMap, конечная станция отслеживается по актуальной орбите. Engine travel/cost/receipt владельцы EP-0001 сохраняются.
2. Прогнать локальный A→B→A и межкластерный A→C→A на seeds 1,2,42 с реальными Buy/Sell quote-bound commands, Undock, Approach, sync, dialogue dock. Пошаговое время, без teleport/ручного IsDocked или подмены денег.
3. Записать стартовую straight estimate и actual arrival отдельно; расходы/выручку брать из VoyageFinanceSnapshot/receipts. Недостижимый предусмотренный рейс — failing evidence; произвольная недостижимая цель не требует изменения Approach speed.
...
Test class: `ClusterVoyageIntegrationTests`. Тестовые сценарии и ожидаемые результаты:

- `ClusterVoyageIntegrationTests.LocalTradeRoundTripThroughCommands` — две остановки и обратный груз с matching authoritative receipts.
- `ClusterVoyageIntegrationTests.InterclusterReturnVoyageThroughCommands` — 2 разных cluster IDs, возврат в исходный, finite budget.
- `ClusterVoyageIntegrationTests.OrbitVoyagePreservesApproachAndLedger` — нет speed multiplier, нет двойной оплаты, actual time отдельно от estimate.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `ClusterVoyageIntegrationTests.LocalTradeRoundTripThroughCommands`, `ClusterVoyageIntegrationTests.InterclusterReturnVoyageThroughCommands`, `ClusterVoyageIntegrationTests.OrbitVoyagePreservesApproachAndLedger` |
| AC-0002 | `ClusterVoyageIntegrationTests.LocalTradeRoundTripThroughCommands`, `ClusterVoyageIntegrationTests.InterclusterReturnVoyageThroughCommands`, `ClusterVoyageIntegrationTests.OrbitVoyagePreservesApproachAndLedger` |
| AC-0003 | `ClusterVoyageIntegrationTests.LocalTradeRoundTripThroughCommands`, `ClusterVoyageIntegrationTests.InterclusterReturnVoyageThroughCommands`, `ClusterVoyageIntegrationTests.OrbitVoyagePreservesApproachAndLedger` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~ClusterVoyageIntegrationTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.ClusterVoyages.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/ClusterVoyageIntegrationTests.cs"
