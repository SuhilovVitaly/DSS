...
ticket: EP-0003-US-0002-TK-0002-full-cluster-content
title: Полная конфигурация 3–5 кластеров
stage: approved
layer: content-data
depends_on: [EP-0003-US-0001-TK-0001-cluster-geography-contract, EP-0003-US-0001-TK-0002-local-cluster-generation, EP-0003-US-0001-TK-0003-local-cluster-content, EP-0003-US-0001-TK-0004-local-cluster-map, EP-0003-US-0002-TK-0001-multi-cluster-placement]
files_touched: 2
...
# Полная конфигурация 3–5 кластеров

## Why

Несколько разных торговых районов: полная конфигурация 3–5 кластеров даёт проверяемый шаг к результату истории. End state: комбинации нижних/верхних границ и five scenario startup.; исходные рынки не продублированы.
...
| tests/DeepSpaceSaga.Client.Tests/FullClusterContentTests.cs | Новый файл; владелец EP-0003-US-0002-TK-0002-full-cluster-content. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Установить clusters.minClusters=3,maxClusters=5; остальные диапазоны сохранить. Проверить 2/5 поясов × 3/5 кластеров × 10/12 станций, наличие всех профилей и воспроизводимость.
2. На content smoke seed 1,2,42 подтвердить успешную генерацию в конечном бюджете. Если min/max profile не вмещается — исправлять только настройки ширины/радиальных резервов с отчётом, не уменьшать требования числа станций или стартового масштаба.
...
Test class: `FullClusterContentTests`. Тестовые сценарии и ожидаемые результаты:

- `FullClusterContentTests.FullClusterBoundaryContent` — комбинации нижних/верхних границ и five scenario startup.
- `FullClusterContentTests.StartClusterRetainsScenarioIdentity` — исходные рынки не продублированы.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `FullClusterContentTests.FullClusterBoundaryContent`, `FullClusterContentTests.StartClusterRetainsScenarioIdentity` |
| AC-0002 | `FullClusterContentTests.FullClusterBoundaryContent`, `FullClusterContentTests.StartClusterRetainsScenarioIdentity` |
| AC-0003 | `FullClusterContentTests.FullClusterBoundaryContent`, `FullClusterContentTests.StartClusterRetainsScenarioIdentity` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~FullClusterContentTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Client/DeepSpaceSaga.Client.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/FullClusterContentTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/FullClusterContentTests.cs"
