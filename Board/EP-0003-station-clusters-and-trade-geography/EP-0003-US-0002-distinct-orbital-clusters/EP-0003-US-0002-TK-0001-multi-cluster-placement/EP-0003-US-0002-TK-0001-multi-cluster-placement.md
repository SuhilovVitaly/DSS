...
ticket: EP-0003-US-0002-TK-0001-multi-cluster-placement
title: Разные орбитальные торговые районы без распада
stage: approved
layer: engine
depends_on: [EP-0003-US-0001-TK-0001-cluster-geography-contract, EP-0003-US-0001-TK-0002-local-cluster-generation, EP-0003-US-0001-TK-0003-local-cluster-content, EP-0003-US-0001-TK-0004-local-cluster-map]
files_touched: 3
...
# Разные орбитальные торговые районы без распада

## Why

Несколько разных торговых районов: разные орбитальные торговые районы без распада даёт проверяемый шаг к результату истории. End state: 3/5 групп, 10/12 станций, profiles представлены.; все pair distances, epochs и critical conjunctions.; 15/35d на старте, atomic failure без ухудшения constraints.
...
| tests/DeepSpaceSaga.Engine.Tests/MultiClusterPlacementTests.cs | Новый файл; владелец EP-0003-US-0002-TK-0001-multi-cluster-placement. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Расширить generator до config.Min/MaxClusters, независимые ordinal streams по cluster ID и attempt. Все группы содержат пять профилей, повторяющиеся роли создают различимую specialization, не менять цены.
2. Назначать group common angular velocity, точные фазовые offsets; жёсткая формация стартовой группы сохраняется. Межкластерные начальные nearest-neighbour distances 15–35d по Vmax, разные пары не обязаны быть в 35d. Если несколько групп в одном поясе — общая angular velocity и разнесённые дуги; для разных radial corridors — disjoint swept envelopes.
3. Проверять t=0,1,7,30,100,365d и аналитические conjunction/opposition relative phases; group distance invariant доказывается общим вращением, separation проверяется outer swept bounds. Не сливать группы и не публиковать partial world при невозможном размещении.
...
Test class: `MultiClusterPlacementTests`. Тестовые сценарии и ожидаемые результаты:

- `MultiClusterPlacementTests.SameSeedDifferentSpecializations` — 3/5 групп, 10/12 станций, profiles представлены.
- `MultiClusterPlacementTests.RigidGroupsFor365Days` — все pair distances, epochs и critical conjunctions.
- `MultiClusterPlacementTests.InitialNeighbourDistancesAndImpossiblePlacement` — 15/35d на старте, atomic failure без ухудшения constraints.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `MultiClusterPlacementTests.SameSeedDifferentSpecializations`, `MultiClusterPlacementTests.RigidGroupsFor365Days`, `MultiClusterPlacementTests.InitialNeighbourDistancesAndImpossiblePlacement` |
| AC-0002 | `MultiClusterPlacementTests.SameSeedDifferentSpecializations`, `MultiClusterPlacementTests.RigidGroupsFor365Days`, `MultiClusterPlacementTests.InitialNeighbourDistancesAndImpossiblePlacement` |
| AC-0003 | `MultiClusterPlacementTests.SameSeedDifferentSpecializations`, `MultiClusterPlacementTests.RigidGroupsFor365Days`, `MultiClusterPlacementTests.InitialNeighbourDistancesAndImpossiblePlacement` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~MultiClusterPlacementTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/StationClusterGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/MultiClusterPlacementTests.cs"
