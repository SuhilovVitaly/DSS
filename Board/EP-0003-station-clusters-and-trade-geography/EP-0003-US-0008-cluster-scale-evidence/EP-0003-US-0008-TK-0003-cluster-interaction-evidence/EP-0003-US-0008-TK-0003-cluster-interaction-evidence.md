...
ticket: EP-0003-US-0008-TK-0003-cluster-interaction-evidence
title: Выбор станций на максимальной сети
stage: approved
layer: client
depends_on: [EP-0003-US-0006-TK-0001-cluster-save-state, EP-0003-US-0006-TK-0002-cluster-local-resume, EP-0003-US-0007-TK-0001-long-voyage-runner, EP-0003-US-0007-TK-0002-long-voyage-diagnostics, EP-0003-US-0008-TK-0002-cluster-performance-report, EP-0002-US-0008-TK-0003-presented-frame-evidence]
files_touched: 1
serves: [AC-0002]
...
# Выбор станций на максимальной сети

## Why

Проверяемая работа полной торговой сети: выбор станций на максимальной сети даёт проверяемый шаг к результату истории. End state: 5*12 IDs, controls не перекрыты.; имя/профиль сохраняют приоритет; manual GPU evidence обязателен.
...
|---|---|---|
| tests/DeepSpaceSaga.Client.Tests/ClusterInteractionEvidenceTests.cs | Новый файл; владелец EP-0003-US-0008-TK-0003-cluster-interaction-evidence. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change. Используется frame diagnostics EP-0002/US-0008.

## Implementation steps

1. На 60 станциях и максимальных ресурсах проверить System→cluster fit→каждая станция→карточка, pointer/zoom/UI scale; использовать production snapshots от integration fixture, не новый генератор в UI.
2. Запустить реальный клиент с DSS_MAP_FRAME_REPORT: max config, включённые пояса/орбиты, панорама и selection; зафиксировать hardware/frames и цель80FPS отдельно от unit tests. Итоговый текст evidence в ticket review, временные артефакты удалить.
...
Test class: `ClusterInteractionEvidenceTests`. Тестовые сценарии и ожидаемые результаты:

- `ClusterInteractionEvidenceTests.EveryStationSelectableAtMaximumNetwork` — 5*12 IDs, controls не перекрыты.
- `ClusterInteractionEvidenceTests.SelectedStationSurvivesLodAndPause` — имя/профиль сохраняют приоритет; manual GPU evidence обязателен.

| Served criterion | Named tests |
|---|---|
| AC-0002 | `ClusterInteractionEvidenceTests.EveryStationSelectableAtMaximumNetwork`, `ClusterInteractionEvidenceTests.SelectedStationSurvivesLodAndPause` |

Coverage: AC-0002 проверяются этими сценариями в пределах end state тикета; остальные слои закрывают критерий своими named tests по Approved ticket map. Fixtures самостоятельные; static oracle не вычислять тем же helper, который тестируется.

Команды из любого cwd (новый tooling test project сначала restore):

```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~ClusterInteractionEvidenceTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Client/DeepSpaceSaga.Client.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/ClusterInteractionEvidenceTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/ClusterInteractionEvidenceTests.cs"
...
- EP-0002-US-0008-TK-0003-presented-frame-evidence: поставляет общий измерительный harness/реальные frame diagnostics; добавленная техническая зависимость нарезки, не смена пользовательского результата.
...
### EP-0003-US-0008-TK-0001-cluster-correctness-corpus

No API change. Engine test corpus, не performance assertions.

### EP-0003-US-0008-TK-0002-cluster-performance-report

CLI: --solar-map --clusters --seeds 1:100 --scenarios all --config max; schemaVersion=1 расширяется optional clusterCounts/economyReportRef, с идентификацией условий.
