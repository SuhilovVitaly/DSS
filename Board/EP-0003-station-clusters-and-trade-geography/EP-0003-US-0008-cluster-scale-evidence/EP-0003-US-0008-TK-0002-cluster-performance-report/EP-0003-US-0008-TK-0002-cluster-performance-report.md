...
ticket: EP-0003-US-0008-TK-0002-cluster-performance-report
title: Стоимость максимальной торговой сети
stage: approved
layer: tooling
depends_on: [EP-0003-US-0006-TK-0001-cluster-save-state, EP-0003-US-0006-TK-0002-cluster-local-resume, EP-0003-US-0007-TK-0001-long-voyage-runner, EP-0003-US-0007-TK-0002-long-voyage-diagnostics, EP-0003-US-0008-TK-0001-cluster-correctness-corpus, EP-0002-US-0008-TK-0002-system-performance-report, EP-0002-US-0008-TK-0003-presented-frame-evidence]
files_touched: 3
...
# Стоимость максимальной торговой сети

## Why

Проверяемая работа полной торговой сети: стоимость максимальной торговой сети даёт проверяемый шаг к результату истории. End state: связь с seed/config/commit проверена.; backend-specific outcome.
...
| tools/DeepSpaceSaga.Performance.Tests/ClusterPerformanceReportTests.cs | Новый файл; владелец EP-0003-US-0008-TK-0002-cluster-performance-report. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Расширить общий --solar-map harness флагом --clusters; использовать 3/5 × 10/12 ×2/5 production config, все сценарии, seeds1..100. Отчёт включает domain counts/resource fields/market counts.
2. Измерить generation/snapshot/save bytes+ms, allocations, render p50/p95/p99; сравнение только со свежим baseline того же host/config. 80FPS verdict допустим только с фактическим client report EP-0002/US-0008, raster помечать отдельно.
3. Прикрепить экономические результаты US-0007 по seed/config/commit; не считать missing report успешной экономической приёмкой.
...
Test class: `ClusterPerformanceReportTests`. Тестовые сценарии и ожидаемые результаты:

- `ClusterPerformanceReportTests.ClusterReportContainsBoundaryAndEconomyEvidence` — связь с seed/config/commit проверена.
- `ClusterPerformanceReportTests.RasterDoesNotClaimGpuFps` — backend-specific outcome.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `ClusterPerformanceReportTests.ClusterReportContainsBoundaryAndEconomyEvidence`, `ClusterPerformanceReportTests.RasterDoesNotClaimGpuFps` |
| AC-0002 | `ClusterPerformanceReportTests.ClusterReportContainsBoundaryAndEconomyEvidence`, `ClusterPerformanceReportTests.RasterDoesNotClaimGpuFps` |
| AC-0003 | `ClusterPerformanceReportTests.ClusterReportContainsBoundaryAndEconomyEvidence`, `ClusterPerformanceReportTests.RasterDoesNotClaimGpuFps` |
...
dotnet test "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/DeepSpaceSaga.Performance.Tests.csproj" --no-restore --filter "FullyQualifiedName~ClusterPerformanceReportTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/DeepSpaceSaga.Performance.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/Program.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/SolarMapEvidence.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/ClusterPerformanceReportTests.cs"
...
- EP-0002-US-0008-TK-0002-system-performance-report: поставляет общий измерительный harness/реальные frame diagnostics; добавленная техническая зависимость нарезки, не смена пользовательского результата.
