...
# Отчёт снабжения и ограничений длительных маршрутов

## Why

Проверяемая экономика длительных путешествий: отчёт снабжения и ограничений длительных маршрутов даёт проверяемый шаг к результату истории. End state: fixture event 6h vs route 20d, понятная диагностика.; убыток/unknown cost basis остаются отчётными фактами.; budget/ledger correctness отдельно от продуктового balance verdict.
...
|---|---|---|
| tools/DeepSpaceSaga.EconomyBalance/ClusterEconomyEvaluator.cs | Новый файл; владелец EP-0003-US-0007-TK-0002-long-voyage-diagnostics. В исходном дереве отсутствует. | Чистая оценка cluster evidence с violations/observations, без новых экономических формул. |
...
| tests/DeepSpaceSaga.EconomyBalance.Tests/LongVoyageDiagnosticsTests.cs | Новый файл; владелец EP-0003-US-0007-TK-0002-long-voyage-diagnostics. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
internal ClusterEconomyEvaluator.Evaluate(BalanceCaseEvidence) возвращает violations/observations; report fields clusterScenario, horizonReachedDays, completedLocalCycles, completedRoundTrips, findings, ownerStoryId. Unknown balance threshold = not-assessed, не passed.

## Implementation steps

1. Сравнить прежние hour-distance bands с новыми day-distance конфигурациями, но не применять старые margin thresholds ко всем кластерам как доказательство баланса. Report outcome разделяет correctness violations и observations без утверждённого profitable target.
2. Добавить diagnostics для stock starvation duration, budget exhaustion, absent consumers/return flow, event duration vs voyage duration, incomplete return и ledger mismatch. Каждой проблеме дать seed/route/epoch/repro и owner EP-0001 story.
3. Не менять station pricing/replenishment/fuel formulas. Отчёт включает все исходные параметры, unsupported/missing observations и отрицательную прибыль без сокрытия. Продуктовые пороги прибыли остаются backlog.
...
Test class: `LongVoyageDiagnosticsTests`. Тестовые сценарии и ожидаемые результаты:

- `LongVoyageDiagnosticsTests.HoursDaysMismatchIsReported` — fixture event 6h vs route 20d, понятная диагностика.
- `LongVoyageDiagnosticsTests.NegativeProfitIsNotRewritten` — убыток/unknown cost basis остаются отчётными фактами.
- `LongVoyageDiagnosticsTests.MissingThresholdIsNotPass` — budget/ledger correctness отдельно от продуктового balance verdict.

| Served criterion | Named tests |
|---|---|
| AC-0002 | `LongVoyageDiagnosticsTests.HoursDaysMismatchIsReported`, `LongVoyageDiagnosticsTests.NegativeProfitIsNotRewritten`, `LongVoyageDiagnosticsTests.MissingThresholdIsNotPass` |
| AC-0003 | `LongVoyageDiagnosticsTests.HoursDaysMismatchIsReported`, `LongVoyageDiagnosticsTests.NegativeProfitIsNotRewritten`, `LongVoyageDiagnosticsTests.MissingThresholdIsNotPass` |
...
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.EconomyBalance.Tests/DeepSpaceSaga.EconomyBalance.Tests.csproj" --no-restore --filter "FullyQualifiedName~LongVoyageDiagnosticsTests"
...
### EP-0003-US-0007-TK-0001-long-voyage-runner
