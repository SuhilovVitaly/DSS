...
# Воспроизводимый отчёт геометрического размещения

## Why

Территории ИИ вокруг доступной торговой сети: воспроизводимый отчёт геометрического размещения даёт проверяемый шаг к результату истории. End state: воспроизводимая геометрия и конечный горизонт.; attempts/reason/seed присутствуют, exit nonzero.
...
| tools/DeepSpaceSaga.Performance.Tests/PlacementEvidenceReportTests.cs | Новый файл; владелец EP-0004-US-0003-TK-0002-placement-evidence-report. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
CLI: --solar-map --clusters --ai-placement --seeds 1:100 --scenarios all --config max. JSON placementChecks, horizonDays=365, criticalEpochs, components, violations.

## Implementation steps

1. Добавить --ai-placement к общему harness; вывести проверки epoch0/1/7/30/100/365 и критические времена, connected component counts, min clearance, attempts и failure seed/config.
2. Получить diagnostics непосредственно из результата AiTradePlacementValidator/генерации через существующий Performance friend assembly; не вычислять отдельную геометрию в tool. Явно пометить «геометрический горизонт, не гарантия будущей безопасности». API diagnostics создаёт предыдущий engine тикет, этот тикет не меняет Engine.
3. Invalid/nonconverged validation не превращать в passed; вывод stable sort epoch/base/link и deterministic checks отдельно от timings.
...
Test class: `PlacementEvidenceReportTests`. Тестовые сценарии и ожидаемые результаты:

- `PlacementEvidenceReportTests.PlacementReportIncludesCriticalEpochs` — воспроизводимая геометрия и конечный горизонт.
- `PlacementEvidenceReportTests.FailedPlacementReportHasRepro` — attempts/reason/seed присутствуют, exit nonzero.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `PlacementEvidenceReportTests.PlacementReportIncludesCriticalEpochs`, `PlacementEvidenceReportTests.FailedPlacementReportHasRepro` |
| AC-0002 | `PlacementEvidenceReportTests.PlacementReportIncludesCriticalEpochs`, `PlacementEvidenceReportTests.FailedPlacementReportHasRepro` |
| AC-0003 | `PlacementEvidenceReportTests.PlacementReportIncludesCriticalEpochs`, `PlacementEvidenceReportTests.FailedPlacementReportHasRepro` |
...
dotnet test "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/DeepSpaceSaga.Performance.Tests.csproj" --no-restore --filter "FullyQualifiedName~PlacementEvidenceReportTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/DeepSpaceSaga.Performance.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/Program.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/SolarMapEvidence.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/PlacementEvidenceReportTests.cs"
