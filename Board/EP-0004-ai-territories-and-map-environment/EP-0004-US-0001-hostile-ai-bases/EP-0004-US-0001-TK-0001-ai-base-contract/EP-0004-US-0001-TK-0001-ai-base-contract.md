...
Различимые враждебные базы ИИ: принадлежность баз и привязки объектов ии даёт проверяемый шаг к результату истории. End state: owner/parent/orbit/IDs и default arrays.; scientific-military human не преобразуется в Ai.
...
| tests/DeepSpaceSaga.Contracts.Tests/AiBaseContractTests.cs | Новый файл; владелец EP-0004-US-0001-TK-0001-ai-base-contract. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить AiMapEnvironmentSnapshot с base descriptors: identity, owner, type, parent planet либо own orbit; Human/Ai — отдельное owner поле, не экономический профиль.
2. Использовать существующие Station/Planet render types и Enemy relation для баз, фактическая классификация AiBase берётся из descriptor. Legacy optional блок пуст; циклических графических типов нет.
...
Test class: `AiBaseContractTests`. Тестовые сценарии и ожидаемые результаты:

- `AiBaseContractTests.AiBaseJsonRoundTrip` — owner/parent/orbit/IDs и default arrays.
- `AiBaseContractTests.MarketProfileDoesNotDefineOwner` — scientific-military human не преобразуется в Ai.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `AiBaseContractTests.AiBaseJsonRoundTrip`, `AiBaseContractTests.MarketProfileDoesNotDefineOwner` |
| AC-0002 | `AiBaseContractTests.AiBaseJsonRoundTrip`, `AiBaseContractTests.MarketProfileDoesNotDefineOwner` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --no-restore --filter "FullyQualifiedName~AiBaseContractTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/DeepSpaceSaga.Contracts.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AiMapEnvironment.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AuthoritativeSnapshot.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/AiBaseContractTests.cs"
