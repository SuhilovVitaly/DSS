...
# Начальные настройки баз ИИ

## Why

Различимые враждебные базы ИИ: начальные настройки баз ии даёт проверяемый шаг к результату истории. End state: counts/radii/attempts конечны, типы доступны.; human cluster counts/stock неизменны.
...
| tests/DeepSpaceSaga.Client.Tests/AiBaseContentTests.cs | Новый файл; владелец EP-0004-US-0001-TK-0004-ai-base-content. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить ai: minBases=2,maxBases=4, defenceRadiusKm=200,patrolRadiusKm=1000,maxPlacementAttempts=64; это явные стартовые настройки для проверки, не утверждение окончательного баланса.
2. Реальная bootstrap проверка seeds1,2,42 на min/max кластерах. Поля радиусов резервируются схемой, начинают отображаться/валидироваться при US-0002; типы Planetary/Orbital известны.
...
Test class: `AiBaseContentTests`. Тестовые сценарии и ожидаемые результаты:

- `AiBaseContentTests.AiContentLoadsAndIsBounded` — counts/radii/attempts конечны, типы доступны.
- `AiBaseContentTests.AiContentKeepsHumanMarkets` — human cluster counts/stock неизменны.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `AiBaseContentTests.AiContentLoadsAndIsBounded`, `AiBaseContentTests.AiContentKeepsHumanMarkets` |
| AC-0002 | `AiBaseContentTests.AiContentLoadsAndIsBounded`, `AiBaseContentTests.AiContentKeepsHumanMarkets` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~AiBaseContentTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Client/DeepSpaceSaga.Client.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/AiBaseContentTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/AiBaseContentTests.cs"
