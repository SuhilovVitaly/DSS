...
Различимые враждебные базы ИИ: авторитетный запрет стыковки и торговли с ии даёт проверяемый шаг к результату истории. End state: direct command и повтор effect validation.; buy/sell/refuel/quote, legacy/quoted, forged dock fixture, no side effects.; same commands работают на human station.
...
| src/DeepSpaceSaga.Engine/SimulationEngine.TradeExecution.cs | src/DeepSpaceSaga.Engine/SimulationEngine.TradeExecution.cs:189 — private bool TryResolveTradeTarget( | Общий hostile-owner gate в quote/legacy/quoted target resolution, до mutation. |
...
| tests/DeepSpaceSaga.Engine.Tests/AuthoritativeHostileAccessTests.cs | Новый файл; владелец EP-0004-US-0001-TK-0003-authoritative-hostile-access. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Единый owner gate по AiMap/base ID добавить в TryStartNavigationCommand и повторную DockPlayerToStation validation; owner не зависит от скрытой кнопки/RelationToPlayer. Отказ station_access_denied до mutation/credits/dialogue grant.
2. TryResolveTradeTarget должен отклонять base для Quote/Buy/Sell/Refuel включая legacy и quoted paths. BuildDockedStationTradeProjection не раскрывает доступный рынок даже при crafted dock state. GrantStationAccess не отменяет Ai gate.
3. Проверить raw commands, effect callback, stale quotes и искусственно docked fixture; untouched credits/cargo/fuel/market revisions/receipts. Human scientific-military доступна по действующим правилам.
...
Test class: `AuthoritativeHostileAccessTests`. Тестовые сценарии и ожидаемые результаты:

- `AuthoritativeHostileAccessTests.RawDockAndDialogueCannotBypassAiGate` — direct command и повтор effect validation.
- `AuthoritativeHostileAccessTests.AllTradeEntrypointsRejectAi` — buy/sell/refuel/quote, legacy/quoted, forged dock fixture, no side effects.
- `AuthoritativeHostileAccessTests.HumanScientificMilitaryStillAccessible` — same commands работают на human station.

| Served criterion | Named tests |
|---|---|
| AC-0003 | `AuthoritativeHostileAccessTests.RawDockAndDialogueCannotBypassAiGate`, `AuthoritativeHostileAccessTests.AllTradeEntrypointsRejectAi`, `AuthoritativeHostileAccessTests.HumanScientificMilitaryStillAccessible` |

Coverage: AC-0003 проверяются этими сценариями в пределах end state тикета; остальные слои закрывают критерий своими named tests по Approved ticket map. Fixtures самостоятельные; static oracle не вычислять тем же helper, который тестируется.

Команды из любого cwd (новый tooling test project сначала restore):

```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~AuthoritativeHostileAccessTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.TradeExecution.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Dialogue/DialogueEffectTransaction.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/AuthoritativeHostileAccessTests.cs"
