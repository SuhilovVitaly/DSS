...
# Выбор базы и различимая принадлежность

## Why

Различимые враждебные базы ИИ: выбор базы и различимая принадлежность даёт проверяемый шаг к результату истории. End state: planet/free base, labels и owner visible.; разные descriptors/commands независимо от цвета.
...
| tests/DeepSpaceSaga.Client.Tests/AiBasePresentationTests.cs | Новый файл; владелец EP-0004-US-0001-TK-0005-ai-base-presentation. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No session API change. ObjectInfoPanelData optional owner/base type строки; AiMapPresentation собирает их из snapshot.

## Implementation steps

1. Нарисовать базу отдельным значком/контуром и подписью owner Ai, используя map descriptor; разрешить selection и info panel. Цвет Enemy дополнить текстом/shape.
2. Не предлагать base как человеческий Trade/Dock UI target, но отказ Engine остаётся обязательным предыдущим тикетом. Не помечать scientific-military Human как AI.
...
Test class: `AiBasePresentationTests`. Тестовые сценарии и ожидаемые результаты:

- `AiBasePresentationTests.AiBaseSelectableAndIdentified` — planet/free base, labels и owner visible.
- `AiBasePresentationTests.HumanScientificMilitaryNotAi` — разные descriptors/commands независимо от цвета.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `AiBasePresentationTests.AiBaseSelectableAndIdentified`, `AiBasePresentationTests.HumanScientificMilitaryNotAi` |
| AC-0003 | `AiBasePresentationTests.AiBaseSelectableAndIdentified`, `AiBasePresentationTests.HumanScientificMilitaryNotAi` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~AiBasePresentationTests"
