...
# Движущиеся области и безопасная стартовая сеть

## Why

Движущиеся области будущей угрозы: движущиеся области и безопасная стартовая сеть даёт проверяемый шаг к результату истории. End state: paused/moving/planet binding/overlap.; middle of segment inside disc обнаруживается.; state diff относительно контрольного мира без зоны.
...
| tests/DeepSpaceSaga.Engine.Tests/MovingTerritoryDataTests.cs | Новый файл; владелец EP-0004-US-0002-TK-0002-moving-territory-data. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Создать один TerritoryMapData на base из config; verify finite0<inner<=outer. Поза центра всегда берётся из актуальной базы, поля радиусов не изменяют RuntimeMotion.
2. До публикации исключить стартового player и все station nodes/segments стартового local graph из outer discs на epoch0; segment-to-disc расстояние учитывать, не только endpoints. Перегенерировать только Ai stage с конечным attempts, не весь human мир.
3. Контрольный проход корабля сквозь disc не создаёт патруль/security incident/урон/расход/принудительную остановку. Сравнивать equal-time control, сохранять обычные fees/market events независимо от географии.
...
Test class: `MovingTerritoryDataTests`. Тестовые сценарии и ожидаемые результаты:

- `MovingTerritoryDataTests.TerritoriesFollowBasesAtAllSpeeds` — paused/moving/planet binding/overlap.
- `MovingTerritoryDataTests.StartGraphSegmentsStayOutside` — middle of segment inside disc обнаруживается.
- `MovingTerritoryDataTests.FlyingThroughTerritoryHasNoGameplayEffect` — state diff относительно контрольного мира без зоны.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `MovingTerritoryDataTests.TerritoriesFollowBasesAtAllSpeeds`, `MovingTerritoryDataTests.StartGraphSegmentsStayOutside`, `MovingTerritoryDataTests.FlyingThroughTerritoryHasNoGameplayEffect` |
| AC-0002 | `MovingTerritoryDataTests.TerritoriesFollowBasesAtAllSpeeds`, `MovingTerritoryDataTests.StartGraphSegmentsStayOutside`, `MovingTerritoryDataTests.FlyingThroughTerritoryHasNoGameplayEffect` |
| AC-0003 | `MovingTerritoryDataTests.TerritoriesFollowBasesAtAllSpeeds`, `MovingTerritoryDataTests.StartGraphSegmentsStayOutside`, `MovingTerritoryDataTests.FlyingThroughTerritoryHasNoGameplayEffect` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~MovingTerritoryDataTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/AiBaseGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/MovingTerritoryDataTests.cs"
