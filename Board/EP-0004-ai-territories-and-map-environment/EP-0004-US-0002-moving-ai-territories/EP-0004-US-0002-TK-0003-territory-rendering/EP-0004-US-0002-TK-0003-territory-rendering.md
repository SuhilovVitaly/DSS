...
# Читаемые радиусы будущей угрозы

## Why

Движущиеся области будущей угрозы: читаемые радиусы будущей угрозы даёт проверяемый шаг к результату истории. End state: exact conversion and moving center.; обоих владельцев можно выбрать, нет active-attack promise.
...
| tests/DeepSpaceSaga.Client.Tests/TerritoryRenderingTests.cs | Новый файл; владелец EP-0004-US-0002-TK-0003-territory-rendering. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No API change. Local drawing from AiMap.Territories + predicted ObjectMotionSnapshot.

## Implementation steps

1. Рисовать оба радиуса в world units radiusKm*10, union appearance полупрозрачной заливкой; per-base контуры и selection сохраняют вклад каждого владельца.
2. Отображать exact radius и текст «территория ИИ; патрули будут добавлены позднее». Route intersection — только геометрическая отметка, без процента нападения.
3. Менять центр вместе с predicted base pose; zoom не меняет мировую границу, наложения под ship/stations/selected marker.
...
Test class: `TerritoryRenderingTests`. Тестовые сценарии и ожидаемые результаты:

- `TerritoryRenderingTests.BothRadiiMatchWorldAtEveryZoom` — exact conversion and moving center.
- `TerritoryRenderingTests.OverlapPreservesSelectionAndInformationalText` — обоих владельцев можно выбрать, нет active-attack promise.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `TerritoryRenderingTests.BothRadiiMatchWorldAtEveryZoom`, `TerritoryRenderingTests.OverlapPreservesSelectionAndInformationalText` |
| AC-0002 | `TerritoryRenderingTests.BothRadiiMatchWorldAtEveryZoom`, `TerritoryRenderingTests.OverlapPreservesSelectionAndInformationalText` |
| AC-0003 | `TerritoryRenderingTests.BothRadiiMatchWorldAtEveryZoom`, `TerritoryRenderingTests.OverlapPreservesSelectionAndInformationalText` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~TerritoryRenderingTests"
