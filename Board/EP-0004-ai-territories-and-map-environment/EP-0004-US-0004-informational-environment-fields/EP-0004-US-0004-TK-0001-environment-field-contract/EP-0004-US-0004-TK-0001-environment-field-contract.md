...
Различимые пространственные поля: геометрия и привязки информационных полей даёт проверяемый шаг к результату истории. End state: parent/orbit/sun, exact dimensions и seed.; immutable informational schema, legacy empty.
...
| tests/DeepSpaceSaga.Contracts.Tests/EnvironmentFieldContractTests.cs | Новый файл; владелец EP-0004-US-0004-TK-0001-environment-field-contract. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить Radiation/Dust/Debris и field descriptors с ID/type/intensity/geometry/decorSeed и one-of привязкой: parent object, own orbit, sun annulus. Все dimensions finite; источник authoritative.
2. Geometry фиксировать circle или annular-sector, не произвольный client mesh. Поле не содержит damage/price/sensor/fuel modifiers.
...
Test class: `EnvironmentFieldContractTests`. Тестовые сценарии и ожидаемые результаты:

- `EnvironmentFieldContractTests.FieldGeometryAndAnchorRoundTrip` — parent/orbit/sun, exact dimensions и seed.
- `EnvironmentFieldContractTests.NoGameplayModifiersInFieldContract` — immutable informational schema, legacy empty.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `EnvironmentFieldContractTests.FieldGeometryAndAnchorRoundTrip`, `EnvironmentFieldContractTests.NoGameplayModifiersInFieldContract` |
| AC-0002 | `EnvironmentFieldContractTests.FieldGeometryAndAnchorRoundTrip`, `EnvironmentFieldContractTests.NoGameplayModifiersInFieldContract` |
| AC-0003 | `EnvironmentFieldContractTests.FieldGeometryAndAnchorRoundTrip`, `EnvironmentFieldContractTests.NoGameplayModifiersInFieldContract` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --no-restore --filter "FullyQualifiedName~EnvironmentFieldContractTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/DeepSpaceSaga.Contracts.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AiMapEnvironment.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/EnvironmentFieldContractTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AiMapEnvironment.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/EnvironmentFieldContractTests.cs"
