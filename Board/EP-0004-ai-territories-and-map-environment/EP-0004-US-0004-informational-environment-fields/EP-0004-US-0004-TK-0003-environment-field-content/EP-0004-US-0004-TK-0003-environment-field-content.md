...
Различимые пространственные поля: настройки трёх типов пространственных полей даёт проверяемый шаг к результату истории. End state: actual loader и resolved map.; unknown modifier property отклоняется strict loader.
...
| tests/DeepSpaceSaga.Client.Tests/EnvironmentFieldContentTests.cs | Новый файл; владелец EP-0004-US-0004-TK-0003-environment-field-content. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить environment: radiationCount=1,dustCount=2,debrisCount=2,radiationRadiusKm=5000,dustWidthKm=100,debrisRadiusKm=250,intensity=0.5. Эти размеры — начальные configurable values; geometry generator проверяет соответствие system/belts.
2. Проверить actual factory startup с field counts/kinds и IDs, не добавляя modifiers или events в экономический content. При invalid размеры ошибка показывает конкретное поле.
...
Test class: `EnvironmentFieldContentTests`. Тестовые сценарии и ожидаемые результаты:

- `EnvironmentFieldContentTests.ThreeFieldKindsInContent` — actual loader и resolved map.
- `EnvironmentFieldContentTests.FieldContentHasNoEffects` — unknown modifier property отклоняется strict loader.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `EnvironmentFieldContentTests.ThreeFieldKindsInContent`, `EnvironmentFieldContentTests.FieldContentHasNoEffects` |
| AC-0002 | `EnvironmentFieldContentTests.ThreeFieldKindsInContent`, `EnvironmentFieldContentTests.FieldContentHasNoEffects` |
| AC-0003 | `EnvironmentFieldContentTests.ThreeFieldKindsInContent`, `EnvironmentFieldContentTests.FieldContentHasNoEffects` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~EnvironmentFieldContentTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Client/DeepSpaceSaga.Client.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/EnvironmentFieldContentTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/EnvironmentFieldContentTests.cs"
