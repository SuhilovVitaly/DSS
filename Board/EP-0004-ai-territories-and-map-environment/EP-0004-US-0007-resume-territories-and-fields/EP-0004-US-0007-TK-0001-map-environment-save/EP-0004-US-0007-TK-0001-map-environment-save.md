...
# Сохранение баз, областей, полей и точек интереса

## Why

Продолжение мира с прежними территориями и полями: сохранение баз, областей, полей и точек интереса даёт проверяемый шаг к результату истории. End state: полный resolved state и следующий motion шаг.; те же owner IDs/phase/seed.; dangling parent/base/radius/cycle/future format отказ без рынка reset.
...
| tests/DeepSpaceSaga.Engine.Tests/MapEnvironmentSaveTests.cs | Новый файл; владелец EP-0004-US-0007-TK-0001-map-environment-save. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить GameStateData.aiMap и serialize resolved AiMapEnvironmentSnapshot целиком: bases/owner/parent/orbits/radii/fields/POI/decoration seeds. Load только восстанавливает, не запускает Ai/field/POI generators.
2. Проверить unique IDs/one-of anchors, valid radii/intensity/geometry, parents/base owners и отсутствие циклов до публикации. Старые solar/cluster saves без aiMap продолжаются без дополнения слоя, future format отклоняется по общей политике.
3. Сравнить continuous vs save/load+advance при overlapped territories, parent-bound fields и current voyage: IDs/geometry/poses/markets/ledger identical; epochs/decor seed не выбираются заново.
...
Test class: `MapEnvironmentSaveTests`. Тестовые сценарии и ожидаемые результаты:

- `MapEnvironmentSaveTests.AiEnvironmentJsonContinuation` — полный resolved state и следующий motion шаг.
- `MapEnvironmentSaveTests.ParentOverlapAndDecorationSurviveLoad` — те же owner IDs/phase/seed.
- `MapEnvironmentSaveTests.InvalidAiSaveIsAtomic` — dangling parent/base/radius/cycle/future format отказ без рынка reset.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `MapEnvironmentSaveTests.AiEnvironmentJsonContinuation`, `MapEnvironmentSaveTests.ParentOverlapAndDecorationSurviveLoad`, `MapEnvironmentSaveTests.InvalidAiSaveIsAtomic` |
| AC-0002 | `MapEnvironmentSaveTests.AiEnvironmentJsonContinuation`, `MapEnvironmentSaveTests.ParentOverlapAndDecorationSurviveLoad`, `MapEnvironmentSaveTests.InvalidAiSaveIsAtomic` |
| AC-0003 | `MapEnvironmentSaveTests.AiEnvironmentJsonContinuation`, `MapEnvironmentSaveTests.ParentOverlapAndDecorationSurviveLoad`, `MapEnvironmentSaveTests.InvalidAiSaveIsAtomic` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~MapEnvironmentSaveTests"
