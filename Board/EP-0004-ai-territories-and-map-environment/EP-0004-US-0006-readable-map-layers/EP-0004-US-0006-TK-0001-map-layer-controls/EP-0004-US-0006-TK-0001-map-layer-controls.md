...
# Переключаемые слои и приоритет взаимодействия

## Why

Управляемые слои полной карты: переключаемые слои и приоритет взаимодействия даёт проверяемый шаг к результату истории. End state: fake session no mutation/remote quote request.; min/max configuration, 1280/1920, UI100/120/150.; owner/radii/type/intensity и informational wording.
...
| tests/DeepSpaceSaga.Client.Tests/MapLayerControlsTests.cs | Новый файл; владелец EP-0004-US-0006-TK-0001-map-layer-controls. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
No session API change. Client-only MapLayerFlags с Orbits/Territories/Fields/PointsOfInterest. Layer flags не сохраняют/не изменяют игровые знания.

## Implementation steps

1. Добавить локальные переключатели Orbits/Territories/Fields/PointsOfInterest в существующую map toolbar; default on, snapshot и Engine состояние неизменны. При выключении слоя убрать его hit tests, сохранить реальные selected objects.
2. Определить порядок: background/belts→fields→territory fills/outlines→planned lines→real markers→selected labels→UI. При совпавших marker IDs cycling selection доступен повторным кликом только в одной screen tolerance; список зависит от visible IDs и сохраняет ship/station priority.
3. System scale показывает clusters/AI territories/major fields; local — base details/intensity/radii. Контуры/значки/подписи дополняют цвет. Camera fit/панели доступны, базовые command UI не перекрыты.
...
Test class: `MapLayerControlsTests`. Тестовые сценарии и ожидаемые результаты:

- `MapLayerControlsTests.ToggleAllLayersPreservesWorldAndQuotes` — fake session no mutation/remote quote request.
- `MapLayerControlsTests.OverlapSelectionAndControlsAtAllScales` — min/max configuration, 1280/1920, UI100/120/150.
- `MapLayerControlsTests.BaseAndFieldInfoStayReadable` — owner/radii/type/intensity и informational wording.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `MapLayerControlsTests.ToggleAllLayersPreservesWorldAndQuotes`, `MapLayerControlsTests.OverlapSelectionAndControlsAtAllScales`, `MapLayerControlsTests.BaseAndFieldInfoStayReadable` |
| AC-0002 | `MapLayerControlsTests.ToggleAllLayersPreservesWorldAndQuotes`, `MapLayerControlsTests.OverlapSelectionAndControlsAtAllScales`, `MapLayerControlsTests.BaseAndFieldInfoStayReadable` |
| AC-0003 | `MapLayerControlsTests.ToggleAllLayersPreservesWorldAndQuotes`, `MapLayerControlsTests.OverlapSelectionAndControlsAtAllScales`, `MapLayerControlsTests.BaseAndFieldInfoStayReadable` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~MapLayerControlsTests"
