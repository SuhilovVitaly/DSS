...
Проверяемая работа карты со всеми слоями: сводный отчёт полной карты со всеми слоями даёт проверяемый шаг к результату истории. End state: обязательные параметры.; honest verdict.; density не скрывается как game objects.
...
| tools/DeepSpaceSaga.Performance.Tests/FullMapPerformanceReportTests.cs | Новый файл; владелец EP-0004-US-0008-TK-0002-full-map-performance-report. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить --all-map-layers поверх общего --solar-map --clusters harness, включить реальные AI/fields/POI. Записать generation/snapshot/save bytes+times, real object vs decorative counts, raster frame distribution и ссылку на actual client frame evidence.
2. Проверить max config с orbit/territory/environment/POI layers on, camera zoom и selection. Отчёт связывает E2/E3 baseline results через commit/config/seed; нет свежего сопоставимого baseline — не заявлять ускорение.
3. Представить итог по каждому criterion passed/failed/not-measured. 80FPS goal и hardware/runtime/vsync обязательны, отсутствие GPU evidence оставляет performance not-measured. Report не доказывает готовность патрулей/боя.
...
Test class: `FullMapPerformanceReportTests`. Тестовые сценарии и ожидаемые результаты:

- `FullMapPerformanceReportTests.FullMapReportRequiresLayerAndBackendContext` — обязательные параметры.
- `FullMapPerformanceReportTests.MissingGpuOrBaselineNotMarkedPassed` — honest verdict.
- `FullMapPerformanceReportTests.DecorationCountsSeparateFromEntities` — density не скрывается как game objects.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `FullMapPerformanceReportTests.FullMapReportRequiresLayerAndBackendContext`, `FullMapPerformanceReportTests.MissingGpuOrBaselineNotMarkedPassed`, `FullMapPerformanceReportTests.DecorationCountsSeparateFromEntities` |
| AC-0002 | `FullMapPerformanceReportTests.FullMapReportRequiresLayerAndBackendContext`, `FullMapPerformanceReportTests.MissingGpuOrBaselineNotMarkedPassed`, `FullMapPerformanceReportTests.DecorationCountsSeparateFromEntities` |
| AC-0003 | `FullMapPerformanceReportTests.FullMapReportRequiresLayerAndBackendContext`, `FullMapPerformanceReportTests.MissingGpuOrBaselineNotMarkedPassed`, `FullMapPerformanceReportTests.DecorationCountsSeparateFromEntities` |
...
dotnet test "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/DeepSpaceSaga.Performance.Tests.csproj" --no-restore --filter "FullyQualifiedName~FullMapPerformanceReportTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/DeepSpaceSaga.Performance.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/Program.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance/SolarMapEvidence.cs" "D:/DeepSpaceSaga/DSS/tools/DeepSpaceSaga.Performance.Tests/FullMapPerformanceReportTests.cs"
