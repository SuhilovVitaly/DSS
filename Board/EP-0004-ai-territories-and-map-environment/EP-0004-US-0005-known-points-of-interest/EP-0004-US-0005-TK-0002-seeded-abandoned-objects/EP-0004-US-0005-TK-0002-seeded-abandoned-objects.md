...
# Генерация заброшенных объектов рядом с ресурсами

## Why

Известные заброшенные объекты среди ресурсов: генерация заброшенных объектов рядом с ресурсами даёт проверяемый шаг к результату истории. End state: repeat source/settings yields same ID/pose/text.; EP-0003 fields IDs/composition/bindings unchanged.; никаких rewards/market mutation.
...
|---|---|---|
| src/DeepSpaceSaga.Engine/Scenario/PointOfInterestGenerator.cs | Новый файл; владелец EP-0004-US-0005-TK-0002-seeded-abandoned-objects. В исходном дереве отсутствует. | Resolved POI descriptors из templates с canonical anchors; не создавать quest/reward mechanics. |
...
| tests/DeepSpaceSaga.Engine.Tests/SeededAbandonedObjectsTests.cs | Новый файл; владелец EP-0004-US-0005-TK-0002-seeded-abandoned-objects. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить PoiTemplates в config, детерминированно разместить по одному template у выбранного canonical resource field/пояса. ID SYS-POI-{ordinal}, stable seed stage, name/description из content, без новой resource collection.
2. POI остаётся отдельным map descriptor, не торговой Station: selection работает по UI region/marker, gameplay commands не получают фиктивный station ID. Привязка через существующий parent/orbit и shared evaluator.
3. Не добавлять exploration/salvage/loot/events и market auto-supply. При visit/selection с одинаковым временем нет новых receipts или side effects.
...
Test class: `SeededAbandonedObjectsTests`. Тестовые сценарии и ожидаемые результаты:

- `SeededAbandonedObjectsTests.PoiSeedIdentityAndText` — repeat source/settings yields same ID/pose/text.
- `SeededAbandonedObjectsTests.ResourcesRetainCanonicalIdentity` — EP-0003 fields IDs/composition/bindings unchanged.
- `SeededAbandonedObjectsTests.PoiVisitCreatesNoGameplayEvent` — никаких rewards/market mutation.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `SeededAbandonedObjectsTests.PoiSeedIdentityAndText`, `SeededAbandonedObjectsTests.ResourcesRetainCanonicalIdentity`, `SeededAbandonedObjectsTests.PoiVisitCreatesNoGameplayEvent` |
| AC-0002 | `SeededAbandonedObjectsTests.PoiSeedIdentityAndText`, `SeededAbandonedObjectsTests.ResourcesRetainCanonicalIdentity`, `SeededAbandonedObjectsTests.PoiVisitCreatesNoGameplayEvent` |
| AC-0003 | `SeededAbandonedObjectsTests.PoiSeedIdentityAndText`, `SeededAbandonedObjectsTests.ResourcesRetainCanonicalIdentity`, `SeededAbandonedObjectsTests.PoiVisitCreatesNoGameplayEvent` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/DeepSpaceSaga.Engine.Tests.csproj" --no-restore --filter "FullyQualifiedName~SeededAbandonedObjectsTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/DeepSpaceSaga.Engine.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/PointOfInterestGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGeneration.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/Scenario/SolarSystemGenerator.cs" "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Engine/SimulationEngine.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Engine.Tests/SeededAbandonedObjectsTests.cs"
