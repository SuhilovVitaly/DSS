...
Известные заброшенные объекты среди ресурсов: известные точки интереса без игровой механики даёт проверяемый шаг к результату истории. End state: unicode тексты и parent/orbit.; backwards compatible optional collection.
...
| tests/DeepSpaceSaga.Contracts.Tests/PoiContractTests.cs | Новый файл; владелец EP-0004-US-0005-TK-0001-poi-contract. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить point-of-interest descriptors с stable object ID/name/description/anchor; reuse OrbitalElements и parent one-of. Ссылки на resources остаются canonical field IDs, копии состава не создавать.
2. Добавить именованные тесты ниже, привязать результаты к served criteria и записать фактические команды/результат в review evidence этого тикета. Нельзя помечать passed ещё не выполненный прогон.
...
Test class: `PoiContractTests`. Тестовые сценарии и ожидаемые результаты:

- `PoiContractTests.PoiDescriptionAndAnchorRoundTrip` — unicode тексты и parent/orbit.
- `PoiContractTests.LegacyMapHasEmptyPoi` — backwards compatible optional collection.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `PoiContractTests.PoiDescriptionAndAnchorRoundTrip`, `PoiContractTests.LegacyMapHasEmptyPoi` |
| AC-0002 | `PoiContractTests.PoiDescriptionAndAnchorRoundTrip`, `PoiContractTests.LegacyMapHasEmptyPoi` |
| AC-0003 | `PoiContractTests.PoiDescriptionAndAnchorRoundTrip`, `PoiContractTests.LegacyMapHasEmptyPoi` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --no-restore --filter "FullyQualifiedName~PoiContractTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/DeepSpaceSaga.Contracts.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AiMapEnvironment.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/PoiContractTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/DeepSpaceSaga.Contracts.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Contracts/AiMapEnvironment.cs" "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Contracts.Tests/PoiContractTests.cs"
