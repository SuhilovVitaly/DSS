...
# Содержимое заброшенных точек интереса

## Why

Известные заброшенные объекты среди ресурсов: содержимое заброшенных точек интереса даёт проверяемый шаг к результату истории. End state: оба текста и IDs проходят loader.; лишние gameplay properties отвергаются.
...
| tests/DeepSpaceSaga.Client.Tests/AbandonedObjectContentTests.cs | Новый файл; владелец EP-0004-US-0005-TK-0003-abandoned-object-content. В исходном дереве отсутствует. | Именованные проверки этого тикета; если csproj — только указанный reference/test setup |
...
1. Добавить два poiTemplates: abandoned-relay / «Заброшенный ретранслятор» / «Неактивный узел связи. Исследование пока недоступно.»; abandoned-platform / «Пустая платформа» / «Покинутая техническая платформа. Здесь пока нет доступных действий.»
2. Проверить непустые уникальные IDs, имена/описания и strict actual config loading; никаких наград/стоимости/ресурсного production в шаблонах.
...
Test class: `AbandonedObjectContentTests`. Тестовые сценарии и ожидаемые результаты:

- `AbandonedObjectContentTests.PoiTemplatesValidAndDistinct` — оба текста и IDs проходят loader.
- `AbandonedObjectContentTests.PoiTemplatesDoNotDefineActions` — лишние gameplay properties отвергаются.

| Served criterion | Named tests |
|---|---|
| AC-0001 | `AbandonedObjectContentTests.PoiTemplatesValidAndDistinct`, `AbandonedObjectContentTests.PoiTemplatesDoNotDefineActions` |
| AC-0003 | `AbandonedObjectContentTests.PoiTemplatesValidAndDistinct`, `AbandonedObjectContentTests.PoiTemplatesDoNotDefineActions` |
...
```powershell
dotnet test "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --no-restore --filter "FullyQualifiedName~AbandonedObjectContentTests"
...
dotnet format "D:/DeepSpaceSaga/DSS/src/DeepSpaceSaga.Client/DeepSpaceSaga.Client.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/AbandonedObjectContentTests.cs"
dotnet format "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/DeepSpaceSaga.Client.Tests.csproj" --verify-no-changes --no-restore --include "D:/DeepSpaceSaga/DSS/tests/DeepSpaceSaga.Client.Tests/AbandonedObjectContentTests.cs"
