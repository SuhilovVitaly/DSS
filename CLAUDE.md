...
├── Trade/                   (TradeScreen + TradeLayout — opened from Station's TRADE button, implemented MVP)
├── Hire/                    (HireScreen + HireLayout — opened from Station's HIRE button, stub; crew hiring specifically)
└── Contracts/               (ContractsScreen + ContractsLayout — opened from Station's CONTRACTS button, stub; passenger contracts, split out of Hire)
```

Shared UI style: `UI/Controls/MenuStyle.cs` (Verdana fonts, DSS button colors, hover/pressed/disabled states).

### Docking (navigation.dock) and the Station screen
...
`StationScreen`'s `TRADE`, `HIRE`, `FINANCE` and `CONTRACTS` buttons are real clickable elements (unlike its other three placeholder lines — Representatives/Install Drilling Unit/Undock). `TRADE` opens the implemented MVP `TradeScreen`; `HIRE`/`CONTRACTS` open `HireScreen`/`ContractsScreen` stub screens, as nested modals on top of `StationScreen` (`SkiaWindow.OpenTradeAsync`/`OpenHireAsync`/`OpenContractsAsync` → `PushModalAsync`, same generic modal-depth mechanism `GameMenu → Save/Load` already uses). `FINANCE` reuses the pre-existing `FinanceScreen` (also reachable from `GameSessionScreen`'s Mechanics panel / Ctrl+F) via the same existing `ScreenEvent.OpenFinance`/`OpenFinanceAsync` — no new screen class needed. Closing any of the four returns to `StationScreen`. `Contracts` was later split out of `Hire` (crew hiring vs. passenger contracts — `HireScreen`'s placeholder text narrowed to "Crew hiring: not available yet" accordingly) and occupies a newly appended row 6, not Hire's old row 4. Each button/placeholder line occupies a fixed body row in `StationLayout` (0..5 original, 6 appended for Contracts, `Station.md`'s button order) — converting a row to a button never repacks the remaining placeholder rows; button height (24 px) is deliberately less than the row height (28 px) so adjacent-row buttons (Trade=row 0, Finance=row 1) don't overlap.
