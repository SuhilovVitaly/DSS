...
`StationScreen`'s `TRADE`, `HIRE` and `FINANCE` buttons are real clickable elements (unlike its other three placeholder lines — Representatives/Install Drilling Unit/Undock). `TRADE`/`HIRE` open `TradeScreen`/`HireScreen`, new stub screens, as nested modals on top of `StationScreen` (`SkiaWindow.OpenTradeAsync`/`OpenHireAsync` → `PushModalAsync`, same generic modal-depth mechanism `GameMenu → Save/Load` already uses). `FINANCE` reuses the pre-existing `FinanceScreen` (also reachable from `GameSessionScreen`'s Mechanics panel / Ctrl+F) via the same existing `ScreenEvent.OpenFinance`/`OpenFinanceAsync` — no new screen class needed. Closing any of the three returns to `StationScreen`. Each button/placeholder line occupies a fixed body row in `StationLayout` (0..5, `Station.md`'s button order) — converting a row to a button never repacks the remaining placeholder rows; button height (24 px) is deliberately less than the row height (28 px) so adjacent-row buttons (Trade=row 0, Finance=row 1) don't overlap.
