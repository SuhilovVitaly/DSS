        ...
        double targetSpeedKmS)
    {
        ...
        while (iterations++ < ApproachTrajectoryMaxIterations)
        {
            ...
            if (step.IsArrived)
            {
                ...
                // THIS LEG WAS PLANNED — for a genuinely moving target that pose is
                // stale by the time the curve is flown, so drawing the route straight
                // into it (the old behavior) showed the ship arriving somewhere the
                // target had long since left. Hand off into the same live-tracking
                // Final-phase pursuit preview used by the Trail-phase hand-off in
                // RunApproachPursuitPreview, re-baking the aim point from the target's
                // state extrapolated forward to THIS instant (mirrors the engine's live
                // re-read — see SimulationEngine.ApplyApproachStep).
