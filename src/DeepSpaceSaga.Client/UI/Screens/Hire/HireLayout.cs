...
public sealed class HireLayout
{
    ...
    public static HireButton HitTest(float screenX, float screenY, int screenWidth, int screenHeight)
    {
        float panelLeft = PanelLeft(screenWidth);
        float panelTop = PanelTop(screenHeight);
        ...
        if (lx >= left && lx <= right && ly >= top && ly <= bottom)
            return HireButton.Close;

        return HireButton.None;
    }
}
