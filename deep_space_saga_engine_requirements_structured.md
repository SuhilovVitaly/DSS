...
- Клик/наведение на объект в пределах `30 px` хит-теста выбирает по приоритету типа (`Station` > корабль игрока > другой корабль > всё остальное), расстояние до курсора — только tie-break внутри одного приоритета.
...
- `42. Подписи объектов на карте`
- `54.9. Object selection: hit-testing, ActiveObject/SelectedObject и приоритет объектов`
...
Назначение: modal-хаб пристыкованного состояния — заглушка первого релиза (открытие/закрытие/пауза, без Trade/Finance/Hire/Undock).

Требования:

- Открывается автоматически после первого успешного `navigation.dock` в сессии (клиент узнаёт об этом только из `CommandResults` следующего снапшота, опрос раз в кадр, edge-triggered по `SnapshotSequence`).
- Открывается повторно левым кликом по станции, к которой корабль пристыкован, либо по самому кораблю игрока, пока он пристыкован (маркеры физически почти совпадают после дока).
- Панель `1400×900` (стандарт игровых окон), закрытие крестиком/`Escape`/кликом по фону — без отмены самой стыковки.
- Modal pause rule (`31.1`, `52`) распространяется как на любое другое модальное окно поверх сессии.

Источники:

- `58.1. Команда navigation.dock`
- `58.2. Открытие экрана станции`
...
### 3.8. Стыковка (Docking)

Назначение: `navigation.dock` — единственная реализованная команда Navigation Computer; MVP первого релиза.

Требования:

- Мгновенное one-shot authoritative действие (не timed `ActiveCycle`), отдельный обработчик от Engine-модуля.
- Валидация по порядку: targetObjectId обязателен и должен существовать → цель должна быть `Station` → дистанция меньше content-driven `rangeKm` (по умолчанию `200`) → скорость/направление корабля синхронизированы со станцией.
- При успехе: корабль физически синхронизируется со станцией (`local offset (1, 1)` world unit, скорость/направление станции), выставляются authoritative `IsDocked`/`DockedStationObjectId`, персистентные через save/load.
- Не реализовано: Undock, блокировка engine-команд корабля во время дока, timed-анимация дока, `navigation.stationsList`.
...
- `SpaceObjectData` включает необязательные поля `isDocked`/`dockedStationObjectId` (`58.1`) — docking state персистится через save/load так же, как остальной `GameState`.
